...
!(function (global, factory) {
    "object" == typeof exports && "undefined" != typeof module
      ...
      : ((global = global || self).wangEditor = factory());
})(window, function () { 
...
DomElement.prototype = {
    ...
    // 格式化视频标签
    formatVideoTag: function formatVideoTag() {
    	//将所有video标签加上preload ="none"属性，让浏览器不加载视频文件
        //this.find("video").attr("preload","none");
        //设置视频封面
        this.find("video").attr("poster",_getBasePath()+'play.png');
    },
    // 视频标签转图片标签
    videoToImg: function videoToImg() {
        ...
        this.find("video").forEach(function (item) {
        	...
        	var node = $('<img class="playerImg" src="'+_getBasePath()+'play.png" data="'+$(item).attr("src")+'"/>');
...
// 替换 pre 标签中的特殊字符
function replacePreSymbol(html) {
    if (html == null) {
        return '';
    }
...
* @returns 
*/
function convertViewportWidth(px) {
    var viewportWidth =  config.viewportWidth; // UI设计稿的宽度
    px = px.replace(/[a-zA-Z]+/g,"");//去掉字母
     
    var number_px = parseInt(px)
    return (100 * number_px / viewportWidth)+'vw';

}

//获取路径  http://localhost:3000/common/default/pc/js/wangeditor/
function _getBasePath() {
    ...
    for (var i = 0, len = els.length; i < len; i++) {
        ...
        if (/^(?=.*\/assets\/)(?=.*\.js).*$/.test(src) || /^(?=.*\/src\/)(?=.*\.ts).*$/.test(src)) {
            ...
            return _getBasePathBefore(value)+'/common/default/pc/js/wangeditor/';
        }
    }
    return '';
}
...
function _getPageBasePath() {  
    ...
    var str = _getBasePath().match(/(\S*)\/common\/default\/pc\/js\/wangeditor\//);
    if(str){
        return str[1]+"/";   
    }
    return '';   
...
DropList.prototype = {
    ...
    show: function show() {
        ...
        if (this._rendered) {
            ...
            $container.css('margin-top', convertViewportWidth(menuHeight + 'px') ).css('width', convertViewportWidth(width + 'px'));
...
Panel.prototype = {
    ...
    show: function show() {
        ...
        if (height) {
            $tabContentContainer.css('height', convertViewportWidth(height + 'px') ).css('overflow-y', 'auto');
        }
...
Link.prototype = {
    ...
    onClick: function onClick(e) {
        ...
        var $linkelem = void 0;
        var config = editor.config;
        var customLink = config.customLink;
        var _this = this;
        //激活焦点
        editor.txt.activateFocus();
        ...
        if (this._active) {
            ...
            if($linkelem.getNodeName() == "A" && $linkelem.attr('linktype') == 'mention'){//@提及 链接不弹出超链接窗口
                return;
            }
            ...
            // 是否显示“删除链接”
            var delBtnDisplay = _this._active ? true : false;

            customLink(function (text, link){
            ...
            },$linkelem.text(),$linkelem.attr('href'),delBtnDisplay);
            ...
            if (editor.selection.isSelectionEmpty()) {
                ...
                },editor.selection.getSelectionText(),'',false);
            }
        }
    },
    /**
    ...
    tryChangeActive: function tryChangeActive(e) {
        ...
        if ($selectionELem.getNodeName() === 'A') {
            ...
            if($selectionELem.attr('linktype') == 'mention'){//@提及 链接不显示本状态
                return;
            }
...
Code.prototype = {
    ...
    onClick: function onClick(e) {
        ...
        var $code = void 0;
        var config = editor.config;
        var customInsertCode = config.customInsertCode;
        var _this = this;
        ...
        if (this._active) {
           ...
           // this._createPanel(language,$startElem.html());
            ...
            customInsertCode(function (_language,code,type){
                
                code = replacePreSymbol(code);
                
                if (type === 'new') {
                    // 新插入
                    _this._insertCode(_language,code);
                } else {
                    // 编辑更新
                    _this._updateCode(_language,code);
                }

            },language,$startElem.html(),'edit');
...
Emoticon.prototype = {
    ...
    onClick: function onClick(e) {
        ...
        var customEmoticon = config.customEmoticon;
        
        if (customEmoticon && typeof customEmoticon === 'function') {
            //激活焦点
            editor.txt.activateFocus();
            customEmoticon();
            // 阻止以下代码执行
        ...
        emotions.forEach(function (emotData) {
            ...
            if (emotType === 'image') {
                content.forEach(function (item) {
                    var alt = item.alt;
                    var src = item.src;
                    var width =convertViewportWidth(item.width);
                   // var width =item.width;
                   // var height = item.height;
                    if (src) {
                        // 加一个 data-w-e 属性，点击图片的时候不再提示编辑图片
                       // faceHtml += '<span class="w-e-item"><img width="' + width + '" height="' + height + '" src="' + src + '" alt="' + alt + '" data-w-e="1"/></span>';
                        faceHtml += '<span class="w-e-item"><span style="display: block;width:' + width + ';"><img _width="' + item.width + '" _height="' + item.height + '" src="' + src + '" alt="' + alt + '" data-w-e="1"/></span></span>';
                ...
                events: [{
                    ...
                    fn: function fn(e) {
                        ...
                        if (nodeName === 'IMG') {
                            ...
                            template.innerHTML = $target.parent().html();
                            var node =  template.childNodes[0];
                          
                            node.setAttribute("width",node.getAttribute("_width"));
                            node.setAttribute("height",node.getAttribute("_height"));
                            node.removeAttribute('_width');
                            node.removeAttribute('_height');
                            console.log("插入图片",template.innerHTML);
                             // 插入图片
                            ...
                            //删除图片的下划线标记
                            var template = document.createElement('div');
                            //  template.innerHTML = $target.html();
                            template.innerHTML = $target.html();

                            // 获取 NodeIterator 对象
                            var nodeIterator = document.createNodeIterator(template, NodeFilter.SHOW_ELEMENT, null, false)
                            // 循环遍历对象的下一个节点
                            var node = null
                            ...
                            insertHtml = '<span>' + template.innerHTML + '</span>';
                        }
...
Video.prototype = {
    ...
    onClick: function onClick() {
        ...
        customUploadVideo(function (link){
            _this._insert(link);
        },_this._insertLinkVideo.bind(_this));
    },
    /** 
    _createPanel: function _createPanel() {
        var _this = this;
    ...
    _insertLinkVideo: function _insertLinkVideo(link) {
    	...
    	//将视频标签转为指定图片标签
    	editor.cmd.do('insertHTML', '<img class="playerImg" src="'+_getBasePath()+'play.png" data="' + link + '" /><p><br></p>');
    	
        //editor.cmd.do('insertHTML', '<video src="'+ link +'" controls="controls" preload="none" poster="'+_getBasePath()+'play.png"/><p><br></p>');
...
Image.prototype = {
    ...
    onClick: function onClick() {
        ...
        var customUploadImage = config.customUploadImage;
        var customDeleteImage = config.customDeleteImage;
        var _this = this;
        var $elem = this.$elem;
        ...
        if (this._active) {
            ...
            //this._createInsertPanel();
            customUploadImage(function (text, url){
                uploadImg.insertLinkImg(url);
            },uploadImg.insertLinkImg.bind(_this));
        }
    },
    /**
    _createEditPanel: function _createEditPanel() {
...
File.prototype = {
 ...
 onClick: function onClick(e) {
     ...
     // 是否显示“删除链接”
     var delBtnDisplay = this._active ? true : false;
     ...
     if (this._active) {
         ...
         if($linkelem.getNodeName() == "A" && $linkelem.attr('linktype') == 'mention'){//@提及 链接不显示文件弹窗
         ...
         // 显示 panel
         //this._createPanel($linkelem.text(), $linkelem.attr('href'));
        customUploadFile(function (text, link){
            _this._insertLink(text, link);
         ...
         if (editor.selection.isSelectionEmpty()) {
            ...
            },editor.selection.getSelectionText(),'',delBtnDisplay);
         }
     }
 },
 /**
...
Hide.prototype = {
	...
	onClick: function onClick(e) {
        ...
        var $elem = this.$elem;
        var config = editor.config;
        var customHide = config.customHide;
	    ...
	    if (this._active) {//选中隐藏标签
	        ...
	        if ($hide != null){
	        	var _visibleType = $($hide).attr('hide-type');
	 	        var _inputValue = $($hide).attr('input-value')
	        	
	 	        //this._createPanel(_visibleType, _inputValue);

                 customHide(function (visibleType, inputValue){
                    //等级标签
                    var gradeTag = "";
                    if(userGradeList != null && userGradeList.length >0){
                        for(var i=0; i<userGradeList.length; i++){
                            var userGrade = userGradeList[i];
                            if(userGrade.needPoint == inputValue){
                    ...
                    if(visibleType == 10){//输入密码可见
                        html = "<hide class='inputValue_10' hide-type='10' input-value='"+inputValue+"'></hide>";
                    }else if(visibleType == 20){//回复话题可见
                        html = "<hide class='inputValue_20' hide-type='20' ></hide>";
                    }else if(visibleType == 30){//达到等级可见
                        html = "<hide class='inputValue_30' hide-type='30' input-value='"+inputValue+"' description='"+gradeTag+"'></hide>";
                    }else if(visibleType == 40){//积分购买可见
                        html = "<hide class='inputValue_40' hide-type='40' input-value='"+inputValue+"'></hide>";
                    }else if(visibleType == 50){//余额购买可见
                        html = "<hide class='inputValue_50' hide-type='50' input-value='"+inputValue+"'></hide>";
                    }
                    ...
                    // 设置同类型标签为相同的值
                    _this._setSameTypeTab(editor.txt.html(),visibleType,inputValue,gradeTag);
                    /**
                    //等级标签
                    var gradeTag = "";
                    if(userGradeList != null && userGradeList.length >0){
                     ...
                     */
                },function (visibleType, inputValue){
                    //等级标签
                    var gradeTag = "";
                    if(userGradeList != null && userGradeList.length >0){
                        for(var i=0; i<userGradeList.length; i++){
                ...
                },_visibleType, _inputValue);

	        }else{
	        	var _visibleType = $selectionELem.attr('hide-type');
		        var _inputValue = $selectionELem.attr('input-value');
	...
	_createPanel: function _createPanel(visibleType,inputValue) {
	    ...
	    var userGradeList = config.userGradeList;
		if(userGradeList != null && userGradeList.length >0){
			userGradeHtml += '<select id="'+inputGradeId+'" style="height:'+convertViewportWidth("16px")+'">';
			for(var i=0; i<userGradeList.length; i++){
				var userGrade = userGradeList[i];
	    ...
	    for(var i=0; i< config.menus.length; i++){
	    	...
	    	template += 		"<span id='"+inputValueBox_40+"' style='display:none;'>需要支付 <input id='"+inputPointId+"' style='width:"+convertViewportWidth('40px')+";text-align:center;' type='text' value='"+inputValue_point+"' maxlength='8'/> 积分可见</span>";
	    	template += 		"<span id='"+inputValueBox_50+"' style='display:none;'>需要支付 ¥<input id='"+inputAmountId+"' style='width:"+convertViewportWidth('40px')+";text-align:center;' type='text' value='"+inputValue_amount+"' maxlength='9'/> 元费用可见</span>";
	    	template += 	"</td>";
	                ...
	                fn: function fn() {
	                	...
	                	if(_visibleTypeName == 10){
	                		 ...
	                		 if(_inputValue.trim() == ""){
                                var tip = config.tip;
                                if (tip && typeof tip === 'function') {
                                    tip(replaceLang(editor, "请输入密码"));//替换多语言
                                }
	                			 return false;
	                		 }
	                		...
	                		if (_inputValue == "" || !/^[0-9]*[1-9][0-9]*$/.test(_inputValue)) {
                                var tip = config.tip;
                                if (tip && typeof tip === 'function') {
                                    tip(replaceLang(editor, "请选择等级"));//替换多语言
                                }
	                			return false;
	                		}
	                		...
	                		if (_inputValue == "" || !/^[0-9]*[1-9][0-9]*$/.test(_inputValue)) {
                                var tip = config.tip;
                                if (tip && typeof tip === 'function') {
                                    tip(replaceLang(editor, "请输入大于0的数字"));//替换多语言
                                }
		     					return false;
		                	}
	                		...
	                		if (_inputValue == "" || !/^(([1-9]\d*)(\.\d{1,2})?)$|(0\.0?([1-9]\d?))$/.test(_inputValue)) {
                                var tip = config.tip;
                                if (tip && typeof tip === 'function') {
                                    tip(replaceLang(editor, "请输入大于0的金额"));//替换多语言
                                }
	                			return false;
	                		}
	                	}
	                ...
	                fn: function fn() {
	                	...
	                	if(_visibleTypeName == 10){
	                		...
	                		if (_inputValue == "" || !/^\d*$/.test(_inputValue)) {
                                var tip = config.tip;
                                if (tip && typeof tip === 'function') {
                                    tip(replaceLang(editor, "请输入数字类型"));//替换多语言
                                }
		     					return false;
		                	}
	                	}else if(_visibleTypeName == 50){
...
Mention.prototype = {
	...
	onClick: function onClick(e) {
        ...
        var customMention = config.customMention;
        
        if (customMention && typeof customMention === 'function') {

            //激活焦点
            editor.txt.activateFocus();

            if(this._active){ // 当前选区在链接里面
                return;
            }

            var $selectionElem = editor.selection.getSelectionContainerElem();
            if ($selectionElem) {
                if($selectionElem.getNodeName() == "A" && $selectionElem.attr('linktype') == 'mention'){
...
AI.prototype = {
	...
	onClick: function onClick(e) {
        ...
        var customAI = config.customAI;
        
        if (customAI && typeof customAI === 'function') {
            //激活焦点
            editor.txt.activateFocus();
            customAI();
            // 阻止以下代码执行
...
MenuConstructors.mention = Mention;
MenuConstructors.ai = AI;
/*
    菜单集合
*/
// 构造函数
...
Text.prototype = {
    ...
    // 插入表情 type:image、emoji
    insertEmoticon: function insertMention(emotHtml,type) {
        var editor = this.editor;

        if(type == 'image'){//图片表情
            var template = document.createElement('div');
            // 加一个 data-w-e 属性，点击图片的时候不再提示编辑图片
            template.innerHTML = '<img src="'+emotHtml+'" data-w-e="1" width="32px" height="32px" />';

            // 插入图片
            var insertHtml = template.innerHTML;

            editor.cmd.do('insertHTML', insertHtml);

        }else{//emoji

            //var insertHtml = '<span>' + emotHtml + '</span>';
    ...
    // 插入@提及
    insertMention: function insertMention(text, link) {
        var editor = this.editor;
        var $textElem = editor.$textElem;
        ...
        if (checkResult === true) {
            editor.cmd.do('insertHTML', '<a href="' + link + '" linktype="mention" target="_blank">' + text + '</a>');
            ...
            // 插入 <span> ，并将选取定位到 <span>
            var $span = $('<span>&#8203;</span>');
            $span.insertAfter($selectionELem);
            editor.selection.createRangeByElem($span, true);
           // editor.selection.restoreSelection();
           // editor.selection.saveRange();
            $span.remove();
           // editor.selection.restoreSelection();// 恢复选区
    ...
    // 插入代码
    insertCode: function insertCode(language,text) {
        var editor = this.editor;
        var cls =' lang-' + language;
        text = replacePreSymbol(text);
        editor.cmd.do('insertHTML', '<br>');
        editor.cmd.do('insertHTML', '<pre class="prettyprint' + cls + '"><code>' + text + '</code></pre><p><br></p>');
    },

    // 插入图片
    insertImage: function insertImage(url,title) {
        var editor = this.editor;

       
        var template = document.createElement('div');
        // 加一个 data-w-e 属性，点击图片的时候不再提示编辑图片
        template.innerHTML = '<img src="'+url+'"/>';
    ...
    activateFocus: function activateFocus() {
        ...
        if ($selectionElem) {
            var nodeName = $selectionElem.getNodeName();
            //console.log("----class---",$selectionElem.selector.getAttribute("class"));
            if(nodeName == "DIV" && editor.txt.html()=="<p><br></p>"){
    ...
    _clearHandle: function _clearHandle() {
        ...
        $textElem.on('keydown', function (e) {
            ...
            if (e.keyCode !== 8) {//如果不是backspace 退格键
                return;
            }
            var $selectionElem = editor.selection.getSelectionContainerElem();
        ...
        $textElem.on('keyup', function (e) {
            ...
            var $selectionElem = editor.selection.getSelectionContainerElem();
            if($selectionElem && $selectionElem.getNodeName() == "A" && $selectionElem.attr('linktype') == 'mention'){
                //退格键删除@提及标签
                $selectionElem.remove();
                editor.selection.restoreSelection();
                e.preventDefault();
    ...
    _pasteHandle: function _pasteHandle() {
        ...
        var $textElem = editor.$textElem;
        var _this = this;
        var customPasteUploadImage = config.customPasteUploadImage;
        // 粘贴图片、文本的事件，每次只能执行一个
        // 判断该次粘贴事件是否可以执行
        var pasteTime = 0;
        function canDo() {
        ...
        $textElem.on('paste', function (e) {
            ...
            customPasteUploadImage(pasteFiles,uploadImg.insertLinkImg.bind(_this));
        });
    },
    ...
    _dragHandle: function _dragHandle() {
        ...
        $textElem.on('drop', function (e) {
            ...
            // 上传图片
            var uploadImg = editor.uploadImg;
            customPasteUploadImage(files,uploadImg.insertLinkImg.bind(_this));
        });
    }
};
...
UploadImg.prototype = {
    ...
    insertLinkImg: function insertLinkImg(link) {
        ...
        img.onerror = function () {
            img = null;
            tip(replaceLang(editor, "插入图片错误")+" "+link);
            // 无法成功下载图片
...
Editor.prototype = {
    ...
    _initDom: function _initDom() {
        ...
        if (textSelector == null) {
            ...
            $textContainerElem.css('border', '1px solid #ccc').css('border-top', 'none').css('height', convertViewportWidth("300px"));
        } else {
     ...
     //   $textElem.attr('contenteditable', 'true').css('width', '100%').css('height', '100%');
        $textElem.attr('contenteditable', 'true').css('height', '100%');
...
//  var inlinecss = '.w-e-toolbar,.w-e-text-container,.w-e-menu-panel {  padding: 0;  margin: 0;  box-sizing: border-box;}.w-e-toolbar *,.w-e-menu-panel * {  padding: 0;  margin: 0;  box-sizing: border-box;}.w-e-clear-fix:after {  content: "";  display: table;  clear: both;}.w-e-toolbar .w-e-droplist {  position: absolute;  left: 0;  top: 0;  background-color: #fff;  border: 1px solid #f1f1f1;  border-right-color: #ccc;  border-bottom-color: #ccc;}.w-e-toolbar .w-e-droplist .w-e-dp-title {  text-align: center;  color: #999;  line-height: 2;  border-bottom: 1px solid #f1f1f1;  font-size: 13px;}.w-e-toolbar .w-e-droplist ul.w-e-list {  list-style: none;  line-height: 1;}.w-e-toolbar .w-e-droplist ul.w-e-list li.w-e-item {  color: #333;  padding: 5px 0;}.w-e-toolbar .w-e-droplist ul.w-e-list li.w-e-item:hover {  background-color: #f1f1f1;}.w-e-toolbar .w-e-droplist ul.w-e-block {  list-style: none;  text-align: left;  padding: 5px;}.w-e-toolbar .w-e-droplist ul.w-e-block li.w-e-item {  display: inline-block;  *display: inline;  *zoom: 1;  padding: 3px 5px;}.w-e-toolbar .w-e-droplist ul.w-e-block li.w-e-item:hover {  background-color: #f1f1f1;}@font-face {  font-family: \'w-e-icon\';  src: url(data:application/font-woff;base64,) format(\'truetype\');  font-weight: normal;  font-style: normal;}[class^="w-e-icon-"],[class*=" w-e-icon-"] {  /* use !important to prevent issues with browser extensions that change fonts */  font-family: \'w-e-icon\' !important;  speak: none;  font-style: normal;  font-weight: normal;  font-variant: normal;  text-transform: none;  line-height: 1;  /* Better Font Rendering =========== */  -webkit-font-smoothing: antialiased;  -moz-osx-font-smoothing: grayscale;}.w-e-icon-close:before {  content: "\\f00d";}.w-e-icon-upload2:before {  content: "\\e9c6";}.w-e-icon-trash-o:before {  content: "\\f014";}.w-e-icon-header:before {  content: "\\f1dc";}.w-e-icon-pencil2:before {  content: "\\e906";}.w-e-icon-paint-brush:before {  content: "\\f1fc";}.w-e-icon-image:before {  content: "\\e90d";}.w-e-icon-hide:before {  content: "\\e98f";}.w-e-icon-play:before {  content: "\\e912";}.w-e-icon-location:before {  content: "\\e947";}.w-e-icon-undo:before {  content: "\\e965";}.w-e-icon-redo:before {  content: "\\e966";}.w-e-icon-quotes-left:before {  content: "\\e977";}.w-e-icon-list-numbered:before {  content: "\\e9b9";}.w-e-icon-list2:before {  content: "\\e9bb";}.w-e-icon-link:before {  content: "\\e9cb";}.w-e-icon-file:before {  content: "\\e900";}.w-e-icon-happy:before {  content: "\\e9df";}.w-e-icon-bold:before {  content: "\\ea62";}.w-e-icon-underline:before {  content: "\\ea63";}.w-e-icon-italic:before {  content: "\\ea64";}.w-e-icon-strikethrough:before {  content: "\\ea65";}.w-e-icon-table2:before {  content: "\\ea71";}.w-e-icon-paragraph-left:before {  content: "\\ea77";}.w-e-icon-paragraph-center:before {  content: "\\ea78";}.w-e-icon-paragraph-right:before {  content: "\\ea79";}.w-e-icon-terminal:before {  content: "\\f120";}.w-e-icon-page-break:before {  content: "\\ea68";}.w-e-icon-cancel-circle:before {  content: "\\ea0d";}.w-e-icon-font:before {  content: "\\ea5c";}.w-e-icon-text-heigh:before {  content: "\\ea5f";}.w-e-toolbar {  display: -webkit-box;  display: -ms-flexbox;  display: flex;  padding: 0 5px;  /* flex-wrap: wrap; */  /* 单个菜单 */}.w-e-toolbar .w-e-menu {  position: relative;  text-align: center;  padding: 5px 10px;  cursor: pointer;}.w-e-toolbar .w-e-menu i {  color: #999;}.w-e-toolbar .w-e-menu:hover i {  color: #333;}.w-e-toolbar .w-e-active i {  color: #1e88e5;}.w-e-toolbar .w-e-active:hover i {  color: #1e88e5;}.w-e-text-container .w-e-panel-container {  position: absolute;  top: 0;  left: 50%;  border: 1px solid #ccc;  border-top: 0;  box-shadow: 1px 1px 2px #ccc;  color: #333;  background-color: #fff;  /* 为 emotion panel 定制的样式 */  /* 上传图片的 panel 定制样式 */}.w-e-text-container .w-e-panel-container .w-e-panel-close {  position: absolute;  right: 0;  top: 0;  padding: 5px;  margin: 2px 5px 0 0;  cursor: pointer;  color: #999;}.w-e-text-container .w-e-panel-container .w-e-panel-close:hover {  color: #333;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title {  list-style: none;  display: -webkit-box;  display: -ms-flexbox;  display: flex;  font-size: 14px;  margin: 2px 10px 0 10px;  border-bottom: 1px solid #f1f1f1;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title .w-e-item {  padding: 3px 5px;  color: #999;  cursor: pointer;  margin: 0 3px;  position: relative;  top: 1px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title .w-e-active {  color: #333;  border-bottom: 1px solid #333;  cursor: default;  font-weight: 700;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content {  padding: 10px 15px 10px 15px;  font-size: 16px;  /* 输入框的样式 */  /* 按钮的样式 */}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input:focus,.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea:focus,.w-e-text-container .w-e-panel-container .w-e-panel-tab-content button:focus {  outline: none;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea {  width: 100%;  border: 1px solid #ccc;  padding: 5px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea:focus {  border-color: #1e88e5;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text] {  border: none;  border-bottom: 1px solid #ccc;  font-size: 14px;  height: 20px;  color: #333;  text-align: left;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text].small {  width: 30px;  text-align: center;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text].block {  display: block;  width: 100%;  margin: 10px 0;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text]:focus {  border-bottom: 2px solid #1e88e5;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button {  font-size: 14px;  color: #1e88e5;  border: none;  padding: 5px 10px;  background-color: #fff;  cursor: pointer;  border-radius: 3px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.left {  float: left;  margin-right: 10px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.right {  float: right;  margin-left: 10px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.gray {  color: #999;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.red {  color: #c24f4a;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button:hover {  background-color: #f1f1f1;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container:after {  content: "";  display: table;  clear: both;}.w-e-text-container .w-e-panel-container .w-e-emoticon-container .w-e-item {  cursor: pointer;  font-size: 18px;  padding: 0 3px;  display: inline-block;  *display: inline;  *zoom: 1;}.w-e-text-container .w-e-panel-container .w-e-up-img-container {  text-align: center;}.w-e-text-container .w-e-panel-container .w-e-up-img-container .w-e-up-btn {  display: inline-block;  *display: inline;  *zoom: 1;  color: #999;  cursor: pointer;  font-size: 60px;  line-height: 1;}.w-e-text-container .w-e-panel-container .w-e-up-img-container .w-e-up-btn:hover {  color: #333;}.w-e-text-container {  position: relative;}.w-e-text-container .w-e-progress {  position: absolute;  background-color: #1e88e5;  bottom: 0;  left: 0;  height: 1px;}.w-e-text {  padding: 0 10px;  overflow-y: scroll;}.w-e-text p,.w-e-text h1,.w-e-text h2,.w-e-text h3,.w-e-text h4,.w-e-text h5,.w-e-text table,.w-e-text pre {  margin: 10px 0;  line-height: 1.5;}.w-e-text ul,.w-e-text ol {  margin: 10px 0 10px 20px;}.w-e-text blockquote {  display: block;  border-left: 8px solid #d0e5f2;  padding: 5px 10px;  margin: 10px 0;  line-height: 1.4;  font-size: 100%;  background-color: #f1f1f1;}.w-e-text code {  display: inline-block;  *display: inline;  *zoom: 1;  background-color: #f1f1f1;  border-radius: 3px;  padding: 3px 5px;  margin: 0 3px;}.w-e-text pre code {  display: block;}.w-e-text table {  border-top: 1px solid #ccc;  border-left: 1px solid #ccc;}.w-e-text table td,.w-e-text table th {  border-bottom: 1px solid #ccc;  border-right: 1px solid #ccc;  padding: 3px 5px;}.w-e-text table th {  border-bottom: 2px solid #ccc;  text-align: center;}.w-e-text:focus {  outline: none;}.w-e-text img {  cursor: pointer;}.w-e-text img:hover {  box-shadow: 0 0 5px #333;}';
    ...
    var inlinecss = '.w-e-toolbar,.w-e-text-container,.w-e-menu-panel {  padding: 0;  margin: 0;  box-sizing: border-box;}.w-e-toolbar *,.w-e-menu-panel * {  padding: 0;  margin: 0;  box-sizing: border-box;}.w-e-clear-fix:after {  content: "";  display: table;  clear: both;}.w-e-toolbar .w-e-droplist {  position: absolute;  left: 0;  top: 0;  background-color: #fff;  border: 1px solid #f1f1f1;  border-right-color: #ccc;  border-bottom-color: #ccc;}.w-e-toolbar .w-e-droplist .w-e-dp-title {  text-align: center;  color: #999;  line-height: 2;  border-bottom: 1px solid #f1f1f1;  font-size: 13px;}.w-e-toolbar .w-e-droplist ul.w-e-list {  list-style: none;  line-height: 1;}.w-e-toolbar .w-e-droplist ul.w-e-list li.w-e-item {  color: #333;  padding: 5px 0;}.w-e-toolbar .w-e-droplist ul.w-e-list li.w-e-item:hover {  background-color: #f1f1f1;}.w-e-toolbar .w-e-droplist ul.w-e-block {  list-style: none;  text-align: left;  padding: 5px;}.w-e-toolbar .w-e-droplist ul.w-e-block li.w-e-item {  display: inline-block;  *display: inline;  *zoom: 1;  padding: 3px 5px;}.w-e-toolbar .w-e-droplist ul.w-e-block li.w-e-item:hover {  background-color: #f1f1f1;}@font-face {  font-family: \'w-e-icon\';  src: url(data:application/font-woff;base64,) format(\'truetype\');  font-weight: normal;  font-style: normal;}[class^="w-e-icon-"],[class*=" w-e-icon-"] {  /* use !important to prevent issues with browser extensions that change fonts */  font-family: \'w-e-icon\' !important;  speak: none;  font-style: normal;  font-weight: normal;  font-variant: normal;  text-transform: none;  line-height: 1;  /* Better Font Rendering =========== */  -webkit-font-smoothing: antialiased;  -moz-osx-font-smoothing: grayscale;}.w-e-icon-close:before {  content: "\\f00d";}.w-e-icon-upload2:before {  content: "\\e9c6";}.w-e-icon-trash-o:before {  content: "\\f014";}.w-e-icon-header:before {  content: "\\f1dc";}.w-e-icon-pencil2:before {  content: "\\e906";}.w-e-icon-paint-brush:before {  content: "\\f1fc";}.w-e-icon-image:before {  content: "\\e90d";}.w-e-icon-hide:before {  content: "\\e901";}.w-e-icon-play:before {  content: "\\e912";}.w-e-icon-location:before {  content: "\\e947";}.w-e-icon-undo:before {  content: "\\e965";}.w-e-icon-redo:before {  content: "\\e966";}.w-e-icon-quotes-left:before {  content: "\\e977";}.w-e-icon-list-numbered:before {  content: "\\e9b9";}.w-e-icon-list2:before {  content: "\\e9bb";}.w-e-icon-link:before {  content: "\\e9cb";}.w-e-icon-file:before {  content: "\\e902";}.w-e-icon-happy:before {  content: "\\e9df";}.w-e-icon-bold:before {  content: "\\ea62";}.w-e-icon-underline:before {  content: "\\ea63";}.w-e-icon-italic:before {  content: "\\ea64";}.w-e-icon-strikethrough:before {  content: "\\ea65";}.w-e-icon-table2:before {  content: "\\ea71";}.w-e-icon-paragraph-left:before {  content: "\\ea77";}.w-e-icon-paragraph-center:before {  content: "\\ea78";}.w-e-icon-paragraph-right:before {  content: "\\ea79";}.w-e-icon-terminal:before {  content: "\\e923";}.w-e-icon-page-break:before {  content: "\\ea68";}.w-e-icon-cancel-circle:before {  content: "\\ea0d";}.w-e-icon-font:before {  content: "\\ea5c";}.w-e-icon-text-heigh:before {  content: "\\ea5f";}.w-e-toolbar {  display: -webkit-box;  display: -ms-flexbox;  display: flex;  padding: 0 5px;  /* flex-wrap: wrap; */  /* 单个菜单 */}.w-e-toolbar .w-e-menu {  position: relative;  text-align: center;  padding: 5px 10px;  cursor: pointer;}.w-e-toolbar .w-e-menu i {  color: #999;}.w-e-toolbar .w-e-menu:hover i {  color: #333;}.w-e-toolbar .w-e-active i {  color: #1e88e5;}.w-e-toolbar .w-e-active:hover i {  color: #1e88e5;}.w-e-text-container .w-e-panel-container {  position: absolute;  top: 0;  left: 50%;  border: 1px solid #ccc;  border-top: 0;  box-shadow: 1px 1px 2px #ccc;  color: #333;  background-color: #fff;  /* 为 emotion panel 定制的样式 */  /* 上传图片的 panel 定制样式 */}.w-e-text-container .w-e-panel-container .w-e-panel-close {  position: absolute;  right: 0;  top: 0;  padding: 5px;  margin: 2px 5px 0 0;  cursor: pointer;  color: #999;}.w-e-text-container .w-e-panel-container .w-e-panel-close:hover {  color: #333;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title {  list-style: none;  display: -webkit-box;  display: -ms-flexbox;  display: flex;  font-size: 14px;  margin: 2px 10px 0 10px;  border-bottom: 1px solid #f1f1f1;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title .w-e-item {  padding: 3px 5px;  color: #999;  cursor: pointer;  margin: 0 3px;  position: relative;  top: 1px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-title .w-e-active {  color: #333;  border-bottom: 1px solid #333;  cursor: default;  font-weight: 700;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content {  padding: 10px 15px 10px 15px;  font-size: 16px;  /* 输入框的样式 */  /* 按钮的样式 */}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input:focus,.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea:focus,.w-e-text-container .w-e-panel-container .w-e-panel-tab-content button:focus {  outline: none;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea {  box-sizing:border-box;-o-box-sizing:border-box;-ms-box-sizing:border-box;-moz-box-sizing:border-box;-webkit-box-sizing:border-box;background: #ffffff;border: 1px solid #eee;outline: none; color: rgba(0,0,0,0.8);border-radius: 3px;line-height: 24px;font-size: 16px;width: 100%;padding: 6px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content textarea:focus {  border-color: #1e88e5;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text] {  border: none;  border-bottom: 1px solid #ccc;  font-size: 14px;  height: 20px;  color: #333;  text-align: left;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text].small {  width: 30px;  text-align: center;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text].block {  display: block;  width: 100%;  margin: 10px 0;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content input[type=text]:focus {  border-bottom: 2px solid #1e88e5;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button {  font-size: 14px;  color: #1e88e5;  border: none;  padding: 5px 10px;  background-color: #fff;  cursor: pointer;  border-radius: 3px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container div.left {  float: left;  margin-right: 10px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container div.left select{height:26px; line-height:26px; padding:2px 4px;border:1px solid;border-color:#e0e0e0 #e0e0e0 #e0e0e0 #e0e0e0;background:#fff;border-radius:2px;margin-right: 1px;margin-top: 3px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.left {  float: left;  margin-right: 10px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.right {  float: right;  margin-left: 10px;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.gray {  color: #999;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button.red {  color: #c24f4a;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container button:hover {  background-color: #f1f1f1;}.w-e-text-container .w-e-panel-container .w-e-panel-tab-content .w-e-button-container:after {  content: "";  display: table;  clear: both;}.w-e-text-container .w-e-panel-container .w-e-emoticon-container .w-e-item {  cursor: pointer;  font-size: 18px;  padding: 0 3px;  display: inline-block;  *display: inline;  *zoom: 1;}.w-e-text-container .w-e-panel-container .w-e-up-img-container {  text-align: center;}.w-e-text-container .w-e-panel-container .w-e-up-img-container .w-e-up-btn {  display: inline-block;  *display: inline;  *zoom: 1;  color: #999;  cursor: pointer;  font-size: 60px;  line-height: 1;}.w-e-text-container .w-e-panel-container .w-e-up-img-container .w-e-up-btn:hover {  color: #333;}.w-e-text-container {  position: relative;}.w-e-text-container .w-e-progress {  position: absolute;  background-color: #1e88e5;  bottom: 0;  left: 0;  height: 1px;}.w-e-text {  padding: 0 10px;  overflow-y: scroll;}.w-e-text p,.w-e-text h1,.w-e-text h2,.w-e-text h3,.w-e-text h4,.w-e-text h5,.w-e-text table,.w-e-text pre {  margin: 10px 0;  line-height: 1.5;}.w-e-text ul,.w-e-text ol {  margin: 10px 0 10px 20px;}.w-e-text blockquote {  display: block;  border-left: 8px solid #d0e5f2;  padding: 5px 10px;  margin: 10px 0;  line-height: 1.4;  font-size: 100%;  background-color: #f1f1f1;}.w-e-text code {  display: inline-block;  *display: inline;  *zoom: 1;  background-color: #f1f1f1;  border-radius: 3px;  padding: 3px 5px;  margin: 0 3px;}.w-e-text pre code {  display: block;}.w-e-text table {  border-top: 1px solid #ccc;  border-left: 1px solid #ccc;}.w-e-text table td,.w-e-text table th {  border-bottom: 1px solid #ccc;  border-right: 1px solid #ccc;  padding: 3px 5px;}.w-e-text table th {  border-bottom: 2px solid #ccc;  text-align: center;}.w-e-text:focus {  outline: none;}.w-e-text img {  cursor: pointer;}.w-e-text img:hover {  box-shadow: 0 0 5px #333;}';
