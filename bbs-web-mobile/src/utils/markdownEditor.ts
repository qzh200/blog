...
import enHans from 'bytemd/locales/en.json'
...
import enMath from '@bytemd/plugin-math/locales/en.json'
import zhHansMerimaid from './mermaid-extension/locales/zh_Hans.json'
import enMerimaid from './mermaid-extension/locales/en.json'
...
import createNotifyHtml from './notifyHtml'
 ...
 * 设置Markdown语言
 */
export const setMarkdownLanguage = () => {
 ...
 * 设置Merimaid语言
 */
const setMerimaidLanguage = () => {
	const store = useStore();
    const {currentLanguage:store_currentLanguage} = storeToRefs(store);
	if(store_currentLanguage.value == 'zh'){//语言 zh,en
		return zhHansMerimaid;
	}
	return enMerimaid;
}
...
export const markdownPlugins= () => {
	return [
		...
		gemoji(),
		gfm({
			locale: setGfmLanguage()
		}),
		//highlight(),
		highlightCode(),//带行号
		
		math({
			locale: setMathLanguage(),
			katexOptions: { 
 ...
 * @param files 
 */
export function showNotifications(files: File[]) {
	let map = new Map<string,any>();
	
	for(let i = 0; i < files.length; i++) {
		let file = files[i];

		let notifyHtml = createNotifyHtml( h('p', null, [
			h('span', { class: 'van-ellipsis' ,style: 'max-width: 220px; display:inline-block;'}, file.name),
			h('span', null, ' '+t('markdownEditor.20')+' '),//上传中
			h('span', { style: 'color: #67C23A', id:'uploadMessage_'+SHA256(file.name) }, ''),
		]))//VNode
		map.set(file.name,notifyHtml)
	}
	return map;
}