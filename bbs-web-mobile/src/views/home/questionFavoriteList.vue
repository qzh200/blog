...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <!--title="问题收藏用户" left-text="返回"-->
                <van-nav-bar :title="t('questionFavoriteList.10')" :left-text="t('common.110')" left-arrow @click-left="onBack" @click-right="router.push({path: '/'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" :error-text="t('common.140')" :finished-text="t('common.150')" @load="onLoad">
                    ...
                    <div v-for="(favorites,index) in state.favoritesList" :key="favorites.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="favorites.account == null || favorites.account == ''" class="cancelAccount">{{t('questionFavoriteList.20')}}</div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = t('questionFavoriteList.10')+' - ' + titleValue;//问题收藏用户
        }
    }, {
