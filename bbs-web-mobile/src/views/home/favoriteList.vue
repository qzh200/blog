...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <!--title="收藏夹" left-text="返回"-->
                <van-nav-bar :title="t('favoriteList.10')" :left-text="t('common.110')" left-arrow @click-left="onBack" @click-right="router.push({path: '/'})">
                    <template #right>
                    ...
                    <div v-for="(favorites,index) in state.favoritesList" :key="favorites.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="left-layout">
                                <div class="topicTag" v-if="favorites.module == 10">{{t('favoriteList.20')}}</div><!-- 话题 -->
                                <div class="questionTag" v-if="favorites.module == 20">{{t('favoriteList.30')}}</div><!-- 问题 -->
                            </div>
                            <div class="right-layout">
                                ...
                                <span class="more">
                                    <Icon name="trash" :size="convertViewportWidth('16px')" @click="onDelete(favorites.id)"/>
                                </span>
                            </div>
                        </div>
                        <div class="bottom-container">
                            <router-link tag="div" v-if="favorites.module == 10" class="content" :to="{path: '/thread', query:{ topicId : favorites.topicId}}">
                                {{favorites.topicTitle}}
                            </router-link>
                             <router-link tag="div" v-if="favorites.module == 20" class="content" :to="{path: '/question', query:{ questionId : favorites.questionId}}">
                                {{favorites.questionTitle}}
                            </router-link>
                        </div>
                    </div>
...
<script lang="ts" setup>
    ...
    import { Favorites, PageView,} from "@/types/index";
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = t('favoriteList.10')+' - ' +titleValue;//收藏夹
        }
    }, {
    ...
    const onRefresh = () => {
        ...
        queryFavoriteList(state.currentpage+1);
    };
    //加载列表
    const onLoad = () => {
        queryFavoriteList(state.currentpage+1);
    ...
    const onDelete = (id: string) => {
        Dialog.confirm({
            message:t('favoriteList.40'),//是否确定删除该项？
        })
        .then(() => {
            ...
            if(response){
                ...
                if(JSON.parse(result.success)){//成功
                    for(let i=0; i<state.favoritesList.length; i++){
                        let favorites = state.favoritesList[i];
                        if(favorites.id == favoriteId){
                            state.favoritesList.splice(i, 1);//删除
                            break;
                        }
                    }
                    Notify({ 
                        type: 'success', 
                        message: t('favoriteList.50')//删除成功
                    });
