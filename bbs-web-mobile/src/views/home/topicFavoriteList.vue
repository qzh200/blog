...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <!--title="收藏用户" left-text="返回"-->
                <van-nav-bar :title="t('topicFavoriteList.10')" :left-text="t('common.110')" left-arrow @click-left="onBack" @click-right="router.push({path: '/'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" :error-text="t('common.140')" :finished-text="t('common.150')" @load="onLoad">
                    ...
                    <div v-for="(favorites,index) in state.favoritesList" :key="favorites.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="favorites.account == null || favorites.account == ''" class="cancelAccount">{{t('topicFavoriteList.20')}}</div>
...
<script lang="ts" setup>
    ...
    import { Favorites, PageView,Topic} from "@/types/index";
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = t('topicFavoriteList.30')+' - ' + titleValue;//话题收藏用户
        }
    }, {
