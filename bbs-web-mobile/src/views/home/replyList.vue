...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <!--title="我的话题回复" left-text="返回"-->
                <van-nav-bar :title="t('replyList.10')" :left-text="t('common.110')" left-arrow @click-left="onBack" @click-right="router.push({path: '/'})">
                    <template #right>
                    ...
                    <div v-for="(reply,index) in state.replyList" :key="reply.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="layout">
                                ...
                                <router-link tag="div" class="link" :to="{path: '/thread', query:{ topicId : reply.topicId}}">
                                    {{reply.topicTitle}}
                                </router-link>
                            </div>
                        </div>
                        <div class="middle-container">
                            <div class="layout">
                                ...
                                <router-link tag="div" class="link" v-if="reply.status == 20" :to="{path: '/thread', query:{ topicId : reply.topicId,commentId:reply.commentId,replyId:reply.id}}">
                                    {{reply.content}}
                                </router-link>
                            </div>
                        </div>
                        <div class="bottom-container">
                            <div class="layout">
                                <!-- [待审核] -->
                                <span class="status" v-if="reply.status == 10">{{t('replyList.20')}}</span>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = t('replyList.10')+' - ' + titleValue;
        }
    }, {
    ...
    const onRefresh = () => {
        ...
        queryReplyList(state.currentpage+1);
    };
    //加载列表
    const onLoad = () => {
        queryReplyList(state.currentpage+1);
