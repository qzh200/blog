...
//允许账号类型
export interface AllowAccountType {
...
export interface Like {
    ...
    module: number;//模块 10:话题  20:评论  30:评论回复  40:问题  50:答案  60:答案回复
    questionId: string;//问题Id
    questionTitle: string;//问题标题

    commentId: string;//评论Id
    commentReplyId: string;//评论回复Id
    answerId: string;//答案Id
    answerReplyId: string;//答案回复Id
    summary: string;//内容摘要
...
export interface AiChatInfo{
    requestId?: string;//系统生成的标识。请求ID
    content: string;//对话内容
    member: number;//对话成员    10.用户 20.AI
}

//AI大模型产品
export interface AiProduct{
    interfaceProduct: number;//接口产品
    name: string;//名称
}
//AI助手插入数据
export interface AiChatInsertData{
    type: number;//类型 0.无 10.纯文本 20.代码 30.图片
    content: string;//内容
    language?: string;//语言
}
//多语言切换选项
...
export interface VoteTheme{
    id: string;//Id
    title: string;//投票标题
    module: number;//模块 10:话题  20:问答
    sourceParameterId: string;//来源参数Id(话题Id、问题Id)
    maxChoice: number;//最大可选数 1:单选  -1:不限制/多选
    createDate: string;//创建时间
    endDate: string;//截止日期
    userName: string;//发起投票用户名称
    status: number;//状态 10:未开始  20.进行中  30.已结束
    isStaff: boolean;//是否为员工 true:员工  false:会员
    voteOptionList:Array<VoteOption>;//投票选项集合
}
//投票选项
