...
<template>
                ...
                <div class="left-container" >
                    <div class="left-navigation" ref="headerNavigation">
                        <el-scrollbar ref="tagScrollbar" :max-height="state.tagScrollbarMaxHeight" wrap-style="transition: max-height 0.3s ease 0s;">
                            <div class="item">
                                <router-link tag="a" :to="{path: '/', query:{navigation:(state.navigationTab == 20 ? 20 :undefined)}}"  class="title" :class="router.currentRoute.value.query.tagId == undefined || router.currentRoute.value.query.tagId == '' ? 'active' :''">
                            ...
                            <div class="item" :ref="'tag_'+tag.id" v-for="tag in state.tagList">
                                ...
                                <el-tooltip effect="tag-customized" :disabled="tagTooltipDisabled" :content="tag.name" placement="right" transition=" "  :hide-after="10">
                                    <router-link v-if="tag.allowRoleViewList != null && tag.allowRoleViewList.length > 0" tag="a" @mouseenter="tagTooltip($event,tag.name)" :to="{path: '/', query:{ tagId : tag.id,navigation:(state.navigationTab == 20 ? 20 :undefined)}}" class="title allowRole" :class="{ 'active': router.currentRoute.value.query.tagId == tag.id, 'showTagArrow':state.showTagArrow == true && tag.childTag.length >0}">
                                    ...
                                    <router-link else tag="a" @mouseenter="tagTooltip($event,tag.name)" :to="{path: '/', query:{ tagId : tag.id,navigation:(state.navigationTab == 20 ? 20 :undefined)}}" class="title" :class="{ 'active': router.currentRoute.value.query.tagId == tag.id, 'showTagArrow':state.showTagArrow == true && tag.childTag.length >0}">
                                        <div class="image-container" v-if="tag.image != null && tag.image != ''">
                                            <el-image :src="tag.image" style="width: 16px; height: 16px;border-radius:2px" fit="cover" />
                                        </div>
                                        <div v-else class="svg-container">
                                ...
                                <div :ref="'childTag_'+childTag.id" class="childTag" :class="state.isExpandTagMap.get(tag.id) == true ? 'expand' :''"  :style="{'max-height': state.isExpandTagMap.get(tag.id) == true ? state.childTagAutoHeightMap.get(tag.id)+'px':0}" v-for="childTag in tag.childTag" >
                                    <el-tooltip effect="tag-customized" :disabled="tagTooltipDisabled" :content="childTag.name" placement="right" transition=" " :hide-after="10">
                                        <router-link v-if="childTag.allowRoleViewList != null && childTag.allowRoleViewList.length > 0" tag="a" @mouseenter="tagTooltip($event,childTag.name)" :to="{path: '/', query:{ tagId : childTag.id,navigation:(state.navigationTab == 20 ? 20 :undefined)}}" class="childTitle allowRole" :class="router.currentRoute.value.query.tagId == childTag.id ? 'active' :''">
                                            
                                            <!-- cell 用js来判断text-overflow是否生效 -->
                                            <div class="text cell">{{childTag.name}}</div>
                                        </router-link>
                                        <router-link else tag="a" @mouseenter="tagTooltip($event,childTag.name)" :to="{path: '/', query:{ tagId : childTag.id,navigation:(state.navigationTab == 20 ? 20 :undefined)}}" class="childTitle" :class="router.currentRoute.value.query.tagId == childTag.id ? 'active' :''">
                                    ...
                                    <div class="allowRoleView">
                                        <template v-for="(allowRoleView,index) in childTag.allowRoleViewList">
                                            <span class="roleItem">{{allowRoleView}}</span>     
                                        </template>
                                    </div>
                                </div>
                ...
                <div class="middle-container">
                    ...
                    <div class="topic-box">
                        <div class="navigation-container">
                            <el-tabs v-model="state.navigationTab" class="navigation-tabs" @tab-click="handleNavigation">
                                <!--label="最新"-->
                                <el-tab-pane :label="t('index.190')" :name="10"></el-tab-pane>
                                <!--label="精华"-->
                                <el-tab-pane :label="t('index.100')" :name="20"></el-tab-pane>
                            </el-tabs>
                        </div>
                        <div class="topicList">
                                ...
                                <div class="content clearfix">
                                    ...
                                    <ul class="info">
                                        <li>
                                            ...
                                            <!--已注销-->
                                            <span v-if="topic.account == null || topic.account == ''" class='cancelNickname'>{{t('index.20')}}</span>   
                                            <span class="userRoleName" v-for="roleName in topic.userRoleNameList">{{roleName}}</span>
                                            <!--官方人员-->
                                            <span class="staff" v-if="topic.isStaff">{{t('index.30')}}</span>
                                        </li>
                                    </ul>
                                    ...
                                    <!--此用户账号已注销-->
                                    <div class="cancelAccount" v-if="topic.account == null || topic.account == ''">{{t('index.40')}}</div>
                                    ...
                                    <h2 class="title clearfix">
                                        ...
                                        <span v-for="(value,key) in topic.hideTagTypeMap" :class="'hideTagType hide_'+key">
                                            ...
                                            <el-tooltip effect="dark" popper-class="popper-transparent" :hide-after="0" :show-after="200" placement="top-start">
                                                <template #content>
                                                    <!--输入密码可见-->
                                                    <span v-if="key == 10">{{t('index.50')}}</span>
                                                    <!--评论话题可见-->
                                                    <span v-if="key == 20">{{t('index.60')}}</span>
                                                    <!--达到等级可见-->
                                        ...
                                        <!--精华-->
                                        <span class="essence" v-if="topic.essence">{{t('index.100')}}</span>
                                        <!--置顶-->
                                        <span class="top" v-if="topic.sort > 0">{{t('index.110')}}</span>
                                        <!--alt="红包" title="红包"-->
                                        <span class="redEnvelope" v-if="topic.giveRedEnvelopeId != null && topic.giveRedEnvelopeId != ''" :alt="t('index.120')" :title="t('index.120')" >
                                            <Icon name="redEnvelope-1" size="24px"/>
                                        </span>
                                        
                                        <!--alt="投票" title="投票"-->
                                        <span class="vote" v-if="topic.voteThemeId != null && topic.voteThemeId != ''" :alt="t('index.200')" :title="t('index.200')" >
                        ...
                        <el-affix target=".affix-container" :offset="state.affix_offset" z-index="1" @scroll="onRightScroll">
                            <div ref="right_ref">
                                ...
                                <div class="postTopicButton">
                                    <!--发话题-->
                                    <el-button type="primary" size="large" @click="router.push({path: '/addTopic'})" plain style="width: 100%;" ><Icon name="topic" size="15px" class="icon"/>{{t('index.130')}}</el-button>
                                </div>
                                <div class="hotTopic" v-if="state.hotTopicList != null && state.hotTopicList.length >0">

                                    <div class="hot-container">
                                        <!--热门话题-->
                                        <div class="title">{{t('index.140')}}</div>
                                        <div class="topicList">
                                            <div class="topicItem" v-for="hotTopic in state.hotTopicList">
                                                ...
                                                <div class="info">
                                                    ...
                                                    <div class="statistic">
                                                        <!--浏览-->
                                                        <div class="viewTotal">{{hotTopic.viewTotal}} {{t('index.150')}}</div>
                                                        <!--评论-->
                                                        <div class="commentCount">{{hotTopic.commentTotal}} {{t('index.160')}}</div>
                                                    </div>
                                                </div>
                                            </div>
    ...
    <div class="main-container wrap linkModule" v-if="state.linkList != null && Object.keys(state.linkList).length>0 && state.linkList.length >0">
        <div class="link-box">
            <!--友情链接-->
            <div class="title">{{t('index.170')}}</div>
            <!-- 有图片 -->
...
import { TabsPaneContext } from 'element-plus';
    ...
    //导航
    const handleNavigation = (tab: TabsPaneContext, event: Event) => {
        if(tab.props.name == 10){
            router.push({ path: "/"})
        }else if(tab.props.name == 20){
            router.push({ path: "/" ,query: { navigation: 20 }})
        }
    }
    ...
    const createVideoPlayer = (mediaInfo:MediaInfo,index:number,topicId:string) => {
        ...
        if(mediaInfo.mediaUrl == null || mediaInfo.mediaUrl == ""){
            if(playerRefValue.querySelector('.dplayer-process') == null){
                ...
                //视频处理中，请稍后再刷新
                dom.innerHTML="<div class='dplayer-process'><div class='box'><div class='prompt'>"+t('index.180')+"</div></div></div>";
    ...
    onBeforeRouteUpdate((to, from, next) => {
        if(to.name == 'index'){
            ...
            if(state.navigationTab == 10){
                queryTopicList(state.tagId,page);
            }else{
                queryTopicEssenceList(state.tagId,page);
            }
    ...
    watchEffect(() => {
        ...
        if(tagId == ''){
            ...
            if(tagList != null && tagList.length >0){
                A:for(let i=0; i<tagList.length; i++ ){
                    let tag = tagList[i];
                    if(tag.id == tagId){
                    ...
                    if(tag.childTag != null && tag.childTag.length >0){
                        for(let j=0; j<tag.childTag.length; j++ ){
                            ...
                            if(childTag.id == tagId){
                                state.head_title = childTag.name
                                state.head_keywords = childTag.name
                                state.head_description = childTag.name
                                break A;
                            }
                        }
                    }
    ...
    onMounted(() => {
        ...
        let navigationTab:number = (router.currentRoute.value.query.navigation != undefined && router.currentRoute.value.query.navigation != '') ? parseInt(router.currentRoute.value.query.navigation as string) :10;
        state.navigationTab = navigationTab;

        queryTagList();
        if(navigationTab == 10){
            queryTopicList(tagId,page);
        }else{
            queryTopicEssenceList(tagId,page);
        }
