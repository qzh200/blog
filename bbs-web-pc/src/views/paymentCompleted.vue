    ...
    <div class="main">
        <div class="main-container wrap backgroundModule main-fullScreen">
            <div class="paymentCompletedModule" >
                <!--title="支付完成"  sub-title="明细信息请到用户中心的余额模块查看"-->
                <el-result icon="success" :title="t('paymentCompleted.10')" :sub-title="t('paymentCompleted.20')">
                    <template #extra>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('paymentCompleted.10')+' - ' + store.state.title,//支付完成
            meta: [
                {
                    name: 'keywords',
                    content: t('paymentCompleted.10')+' - ' + store.state.title//支付完成
                },{
                    name: "description",
                    content: t('paymentCompleted.10')+' - ' + store.state.title//支付完成
                }
