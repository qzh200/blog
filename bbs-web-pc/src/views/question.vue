        ...
        <div class="main-container wrap">
            ...
            <el-affix position="top" :offset="140">
                <div class="leftFloatPanel">
                    <el-badge :value="(Long.fromString(state.likeCount).gt(99999) ? '99999+' : state.likeCount)" :class="state.alreadyLiked == true ? 'active':''">
                        <!-- content="点赞" -->
                        <el-tooltip effect="tag-customized-grey" :content="t('question.670')" placement="right-end" :hide-after="10">
                            <div class="circle" @click="()=>{state.alreadyLiked == true ? deleteLike(state.question.id,40) : addLike(state.question.id,40)}">
                    ...
                    <el-badge :value="(Long.fromString(state.favoriteCount).gt(99999) ? '99999+' : state.favoriteCount)" :class="state.alreadyCollected == true ? 'active':''">
                        <!-- content="收藏" -->
                        <el-tooltip effect="tag-customized-grey" :content="t('question.120')" placement="right-end" :hide-after="10">
                            <div class="circle" @click="()=>{state.alreadyCollected == true ? deleteFavorite(state.question.id):addFavorite(state.question.id)}">
                    ...
                    <el-badge :value="state.totalrecord" :max="99999" class="item">
                        <!-- content="答案" -->
                        <el-tooltip effect="tag-customized-grey" :content="t('question.660')" placement="right-end" :hide-after="10">
                            <div class="circle" @click="onScrollToAnswerWindow">
            ...
            <div class="questionContentModule">
	            ...
	            <div class="left" v-if="state.question != undefined && state.question != null && Object.keys(state.question).length>0">
                    <div class="question-wrap">
                        ...
                        <div class="rightInfo">
                            <div class="report" v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0" @click="addReportUI(state.questionId,40)">
                                <!--举报-->
                                <span><Icon name="error-warning-line" size="16px" class="icon"/>{{t('question.10')}}</span>
                            </div>
                            <div class="appendQuestion" v-if="state.question.userName == $store.state.systemUser.userName">
                                <!--追加提问-->
                                <span @click="appendQuestionUI(state.questionId);"> <Icon name="append" size="16px" class="icon"/>{{t('question.20')}}</span>
                            </div>
                            <!--只有中文显示IP归属地-->
                        ...
                        <div class="questionBox">
                            <div class="title">
                                ...
                                <!-- 置顶
                                <span class="top" v-if="state.question.sort > 0">{{t('question.680')}}</span> -->
                            </div>
                                    ...
                                    <el-form label-position="right" :ref="'formAppendQuestionRef'" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                        <el-form-item :error="error.appendQuestionContent">
                                            ...
                                            <div v-if="state.isAppendQuestionMarkdown" style="width: 100%;">
                                                <!--placeholder="请输入内容..."-->
                                                <Editor  :editorId="'appendQuestion'" :value="state.appendQuestionMarkdownContent" :plugins="state.appendQuestionEditorPlugin" :locale="setMarkdownLanguage()" :sanitize="appendQuestionSanitize" :placeholder="t('question.30')" @change="handleAppendQuestionMarkdownChange"/>
                                            </div>
                                        ...
                                        <el-form-item :error="error.captchaValue.get('appendQuestion-'+state.questionId)" v-if="state.showCaptcha.get('appendQuestion-'+state.questionId)" class="captcha-item">
                                            <el-row align="middle">
                                                <el-col :span="12" >
                                                    <!--placeholder="验证码"-->
                                                    <el-input  v-model="state.captchaValue['appendQuestion-'+state.questionId]" @change.native="checkCaptchaValueRules('appendQuestion-'+state.questionId)"  class="captchaInput" maxlength="4" :placeholder="t('common.70')" clearable >
                                                        <template #prefix>
                                                ...
                                                <el-col :span="3" :offset="1">
                                                    <!--换一幅-->
                                                    <el-link type="primary" @click="replaceCaptcha('appendQuestion-'+state.questionId)" :underline="false" class="replaceCaptchaText">{{t('common.80')}}</el-link>
                                                </el-col>
                                            </el-row>
                                        </el-form-item>
                                        <el-form-item :error="error.question" class="form-action">
                                            <!--提交-->
                                            <el-button class="submitButton" type="primary"  @mousedown.native="onAppendQuestionFormSubmit()" :disabled="state.allowSubmit.get('appendQuestion-'+state.questionId)">{{t('common.50')}}</el-button>
                                            <!--取消-->
                                            <el-button class="submitButton" type="primary"  @mousedown.native="onCancelAppendQuestion()" plain>{{t('common.60')}}</el-button>
                                        </el-form-item>
                                    </el-form>
                                </div>
                            </div>
                            ...
                            <div class="questionInfo clearfix" >
                                ...
                                <!--次阅读-->
                                <div class="viewTotal">{{t('question.40',{'p1': state.question.viewTotal})}}</div>
                                <div class="answer-icon"><Icon name="commentCount" size="16px" class="icon"/></div>
                                <!--个回答-->
                                <div class="answer">{{t('question.50',{'p1': state.question.answerTotal})}}</div>

                                <div class="ribbon-container" v-if="state.question.sort >0">
                                    <div class="fold" >{{t('question.680')}}</div><!-- 置顶 -->
                                </div>
                            </div>
                            ...
                            <div class="reward" v-if="parseFloat(state.question.amount) > 0 || Long.fromString(String(state.question.point)).gt(0)">
                                <div class="info" >
                                    ...
                                    <template v-if="parseFloat(state.question.amount) > 0 && Long.fromString(state.question.point).lte(0)">
                                        <i18n-t keypath="question.60" scope="global">
                                            <template #p1>
                                    ...
                                    <template v-if="parseFloat(state.question.amount) <=0 && Long.fromString(state.question.point).gt(0)">
                                        <i18n-t keypath="question.70" scope="global">
                                            <template #p1>
                                    ...
                                    <!-- 含有金额和积分 -->
                                    <template v-if="parseFloat(state.question.amount) >0 && Long.fromString(state.question.point).gt(0)">
                                        <i18n-t keypath="question.80" scope="global">
                                            <template #p1>
                                                <span class="symbol">¥</span><span class="amount">{{state.question.amount}}</span>
                                            </template>
                                            <template #p2>
                            ...
                            <div :class="[state.question.isMarkdown != null && state.question.isMarkdown == true ? 'markdown-body-custom questionMarkdownContent' : 'content', router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 40 ? 'reportMark' : '']" >
                                <!--此用户账号已注销-->
                                <div class="cancelAccount" v-if="state.question.account == null || state.question.account == ''">{{t('question.90')}}</div>
                            ...
                            <template v-for="(appendQuestionItem,index) in state.question.appendQuestionItemList">
                                <div :class="(index%2)==0 ? 'appendBox odd' : 'appendBox even'">
                                    <div class="head">
                                        <!--第{{index + 1}}条附言-->
                                        <span class="prompt">{{t('question.100',{'p1': index + 1})}}</span>
                            ...
                            <div class="favorite-formModule" v-if="state.question.userName != null && state.question.userName != ''">
                                <!--content="点赞"-->
                                <el-tooltip effect="tag-customized-grey" :content="t('question.670')" placement="top" :hide-after="10">
                                    <div class="btn-item" :class="state.alreadyLiked == true ? 'active' : ''">
                                        <div class="circle" @click="()=>{state.alreadyLiked == true ? deleteLike(state.question.id,40) : addLike(state.question.id,40)}">
                                            <Icon name="thumb-up-fill" size="26px" class="icon"/>
                                        </div>
                                        <!--赞-->
                                        <div class="text">{{state.likeCount}} {{t('question.140')}}</div>
                                    </div>
                                </el-tooltip>
                                <!--content="收藏"-->
                                <el-tooltip effect="tag-customized-grey" :content="t('question.120')" placement="top" :hide-after="10">
                                    <div class="btn-item" :class="state.alreadyCollected == true ? 'active' : ''">
                                        <div class="circle" @click="()=>{state.alreadyCollected == true ? deleteFavorite(state.question.id):addFavorite(state.question.id)}">
                                            <Icon name="star-fill" size="26px" class="icon"/>
                                        </div>
                                        <!--收藏-->
                                        <div class="text" >{{state.favoriteCount}} {{ t('question.120') }}</div>
                                    </div>
                                </el-tooltip>
                            </div>
                    ...
                    <!-- 投票 -->
					<div class="voteModule" v-if="state.question != undefined && state.question != null  && Object.keys(state.question).length >0 && state.voteTheme != undefined && state.voteTheme != null && Object.keys(state.voteTheme).length >0">
                            ...
                            <div v-for="voteOption in state.voteTheme.voteOptionList">
                                ...
                                <el-progress :text-inside="true" :stroke-width="24" :percentage="state.votePercentage.get(voteOption.id)" :color="voteOption.selected ==true ? '#8cc4ff' :'#c9cdd4'" class="progress" :class="{'active': voteOption.selected}" @click="addVoteRecord(voteOption)">
                                    <template #default="{ percentage }">
                                        ...
                                        <span class="percentage-label">{{t('question.700',{'p1': voteOption.totalVotes})}}</span>
                        ...
                        <div class="voteInfo">
                            <!-- '单选' : '多选' -->
                            <div class="options">{{state.voteTheme.maxChoice == 1 ? t('question.710') : t('question.720')}}</div>
                            <div class="status">
                                <!--投票未开始-->
                                <span v-if="state.voteTheme.status == 10">{{t('question.730')}}</span>
                                <!--{{state.voteTheme.endDate}}截止投票-->
                                <span v-if="state.voteTheme.status == 20 && (state.voteTheme.endDate != null && state.voteTheme.endDate != '')">{{t('question.740',{'p1': state.voteTheme.endDate})}}</span>
                                <!--投票进行中-->
                                <span v-if="state.voteTheme.status == 20 && (state.voteTheme.endDate == null || state.voteTheme.endDate == '')">{{t('question.750')}}</span>
                                <!--投票已结束-->
                                <span v-if="state.voteTheme.status == 30">{{t('question.760')}}</span>
                            </div>
                            <!--共{{ state.voterCount}}票-->
                            <div class="quantity">{{t('question.770',{'p1': state.voterCount})}}</div>
                        </div>
                    </div>
                    ...
                    <div v-if="state.question.allow">
                        ...
                        <div v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0">
                                    ...
                                    <el-form label-position="right" :ref="'formAddAnswerRef'" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                        <el-form-item :error="error.content.get('addAnswer-'+state.questionId)">
                                            ...
                                            <div v-if="state.isAddAnswerMarkdown" style="width: 100%;">
                                                <!-- placeholder="请输入内容..." -->
                                                <Editor :editorId="'addAnswer'" :value="state.addAnswerMarkdownContent" :plugins="state.addAnswerEditorPlugin" :locale="setMarkdownLanguage()" :sanitize="answerSanitize" :placeholder="t('question.30')" @change="handleAddAnswerMarkdownChange"/>
                                        ...
                                        <el-form-item :error="error.captchaValue.get('addAnswer-'+state.questionId)" v-if="state.showCaptcha.get('addAnswer-'+state.questionId)" class="captcha-item">
                                            <el-row align="middle">
                                                <el-col :span="12" >
                                                    <!--placeholder="验证码"-->
                                                    <el-input  v-model="state.captchaValue['addAnswer-'+state.questionId]" @change.native="checkCaptchaValueRules('addAnswer-'+state.questionId)"  class="captchaInput" maxlength="4" :placeholder="t('common.70')" clearable >
                                                        <template #prefix>
                                                            <Icon name="shield-check-line" size="16px"/>
                                                        </template>
                                                    </el-input>
                                                ...
                                                <el-col :span="3" :offset="1">
                                                    <!--换一幅-->
                                                    <el-link type="primary" @click="replaceCaptcha('addAnswer-'+state.questionId)" :underline="false" class="replaceCaptchaText">{{t('common.80')}}</el-link>
                                                </el-col>
                                            </el-row>
                                        </el-form-item>
                                        <el-form-item :error="error.answer.get('addAnswer-'+state.questionId)" class="form-action">
                                            <!--提交-->
                                            <el-button class="submitButton" type="primary"  @mousedown.native="onAddAnswerFormSubmit" :disabled="state.allowSubmit.get('addAnswer-'+state.questionId)">{{t('common.50')}}</el-button>
                                            <div class="promptInfo">
                                                <!-- 我来说两句，分享我的答案 -->
                                                <h2>{{t('question.160')}}</h2>
                                            </div>
                                        </el-form-item>
                                    </el-form>
                                </div>
                            ...
                            <!-- 不显示回答表单 -->       
                            <div class="addAnswerModule" v-if="state.isLoadingComplete && !state.allowAnswer">
                                <div class="addAnswer-wrap">
                                    <div class="respond">
                                        <!-- 回答已关闭 -->
                                        <p>
                        ...
                        <div class="addAnswerModule" v-if="$store.state.systemUser == null || Object.keys($store.state.systemUser).length==0">
                            <div class="addAnswer-wrap">
                                <div class="respond">
                                    <p >
                                        <!-- 请登录后回答。没有帐号？注册一个。-->
                                        <i18n-t keypath="question.180" scope="global">
                                            <template #p1>
                                                <router-link tag="a" :to="{path: '/login',query:{jumpUrl :createJumpAttribute()}}"> {{t('question.190')}}</router-link>
                                            </template>
                                            <template #p2>
                                                <router-link tag="a" :to="'/register'">{{t('question.200')}}</router-link>
                                            </template>
                                        </i18n-t>
                                    </p>
                                </div>
                            ...
                            <div :class="[answer.adoption == true ? 'answer active' : 'answer',router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 50 && answer.id == router.currentRoute.value.query.answerId ? 'reportMark' : '']" v-for="(answer,index) in state.answerList" :key="answer.id"  :ref="'answer_'+answer.id" v-lazy-tag="()=>{onQueryLike(answer.id,50)}">
                                ...
                                <div class="answer-author">
                                    ...
                                    <div class="info-container">
                                        <router-link tag="a" class="userName" :to="{path:'/user/control/home',query: {userName: answer.userName}}">
                                         ...
                                         <!-- 已注销 -->
                                        <span v-if="answer.account == null || answer.account == ''" class='cancelNickname'>{{t('question.220')}}</span> 
                                        <span class="userRoleName" v-for="roleName in answer.userRoleNameList">{{roleName}}</span>
                                        <!-- 官方人员 -->
                                        <span class="staff" v-if="answer.isStaff">{{t('question.230')}}</span>
                                        <!-- 作者 -->
                                        <span class="master" v-if="answer.userName == state.question.userName && answer.isStaff == state.question.isStaff">{{t('question.240')}}</span>
                                    </div> 
                                </div>
                                ...
                                <div class="adoption">
                                    <div class="ribbon-wrapper">
                                        <!-- 最佳答案 -->
                                        <div class="ribbon">{{t('question.250')}}</div>
                                    </div>
                                    <!--  
                                ...
                                <div :class="[answer.isMarkdown != null && answer.isMarkdown == true ? 'markdown-body-custom answerMarkdownContent' : 'answerContent']" >
                                    <!-- 此用户账号已注销 -->
                                    <div class="cancelAccount" v-if="answer.account == null || answer.account == ''">{{t('question.90')}}</div>
                                ...
                                <div class="replyList" :class="'affix-container_reply_'+answer.id" v-if="answer.answerReplyList != null && answer.answerReplyList.length >0">
                                    <ul class="timeline box">
                                        <!-- 延迟加载点赞 -->
                                        <li class="timeline-item replyItem-container" v-for="(reply,index2) in answer.answerReplyList" :key="reply.id" :replyId="reply.id" :ref="handleReplyNodes" v-lazy-tag="()=>{onQueryLike(reply.id,60)}" v-show="state.replyExpandOrShrink.get(answer.id) || (state.replyExpandOrShrink.get(answer.id) == false && index2 <state.viewNumber)">
                                                ...
                                                <div :class="router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 60 && reply.id == router.currentRoute.value.query.replyId ? 'reply-reportMark' : ''">
                                                    <div class="reply-top" >
                                                        <div class="reply-author">
                                                            ...
                                                            <!--已注销-->
                                                            <span v-if="reply.account == null || reply.account == ''" class='cancelNickname'>{{t('question.220')}}</span>   
                                                            ...
                                                            <!--官方人员-->
                                                            <span class="staff" v-if="reply.isStaff">{{t('question.230')}}</span>
                                                             <!--作者-->
                                                            <span class="master" v-if="reply.userName == state.question.userName && reply.isStaff == state.question.isStaff">{{t('question.240')}}</span>
                                                        ...
                                                        <div class="friendInfo" v-if="reply.friendUserName != null && reply.friendUserName != ''">
                                                            ...
                                                            <h2 class="nameInfo" >
                                                                <!--已注销-->
                                                                <span v-if="reply.friendAccount == null || reply.friendAccount == ''" class="cancelNickname">{{t('question.220')}}</span>  
                                                                ...
                                                                <router-link tag="a" v-if="reply.friendAccount != null && reply.friendAccount != ''" class="userName" :to="{path:'/user/control/home',query: {userName: reply.friendUserName}}">
                                                                 ...
                                                                 <!--作者-->
                                                                <span class="master" v-if="reply.friendAccount != null && reply.friendAccount != '' && reply.friendUserName == state.question.userName && reply.isFriendStaff == state.question.isStaff">{{t('question.240')}}</span>
                                                        ...
                                                        <span class="ipAddress">
                                                            ...
                                                            <a v-if="state.alreadyLikedAnswerReply.get(reply.id) != undefined && state.alreadyLikedAnswerReply.get(reply.id) == true" @click="deleteLike(reply.id,60)">
                                                                <!--赞-->
                                                                <Icon name="heart-fill" size="15px" class="like-icon"/><span class="like-quantity">{{state.answerReplyLikeCount.get(reply.id) != undefined  && Long.fromString(String(state.answerReplyLikeCount.get(reply.id))).gt(0) ? state.answerReplyLikeCount.get(reply.id) : ''}}</span><span class="like-text">{{t('question.140')}}</span>
                                                            </a>
                                                            <a v-else @click="addLike(reply.id,60)">
                                                                <!--赞-->
                                                                <Icon name="heart" size="15px" class="like-icon"/><span class="like-quantity">{{state.answerReplyLikeCount.get(reply.id) != undefined  && Long.fromString(String(state.answerReplyLikeCount.get(reply.id))).gt(0) ? state.answerReplyLikeCount.get(reply.id) : ''}}</span><span class="like-text">{{t('question.140')}}</span>
                                                            </a>
                                                            <el-dropdown>
                                                                <div class="more">
                                                                ...
                                                                <template #dropdown>
                                                                    <el-dropdown-menu>
                                                                        <!--回复-->
                                                                        <el-dropdown-item v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0"  @click="addReplyFriendUI(answer.id,reply.id)"><i class="dropdown-menu-icon" ><icon name="reply" size="15px"/></i> {{t('question.260')}} </el-dropdown-item>
                                                                        <!--举报-->
                                                                        <el-dropdown-item v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0" @click="addReportUI(reply.id,60)"><i class="dropdown-menu-icon" ><icon name="error-warning-line" size="15px"/></i> {{t('question.10')}} </el-dropdown-item>
                                                                        
                                                                        <!--编辑-->
                                                                        <el-dropdown-item v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0 && reply.userName ==  $store.state.systemUser.userName" @click="editReplyUI(reply)"><i class="dropdown-menu-icon" ><icon name="pencil-alt" size="15px"/></i> {{t('question.270')}} </el-dropdown-item>
                                                                        <!--删除-->
                                                                        <el-dropdown-item v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0 && reply.userName ==  $store.state.systemUser.userName" @click="onDeleteReply(answer.id,reply.id)"><i class="dropdown-menu-icon" ><icon name="trash" size="15px"/></i> {{t('question.280')}} </el-dropdown-item>
                                                                    </el-dropdown-menu>
                                                                </template>
                                                            </el-dropdown>
                                                        </span>
                                                    ...
                                                    <div class="replyContent" @click="clickReplyLevel(answer.id,reply.id)">
                                                        <!--此用户账号已注销-->
                                                        <div v-if="reply.account == null || reply.account == ''" class="cancelAccount">{{t('question.90')}}</div>
                                                        ...
                                                        <div class="addReplyFriend-wrap">
                                                            <el-form label-position="right" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                                                ...
                                                                <el-form-item :error="error.reply.get('addReplyFriend-'+reply.id)">
                                                                    <!--提交-->
                                                                    <el-button class="submitButton" type="primary" @mousedown.native="onAddReplyFriend(answer.id,reply.id)" :disabled="state.allowSubmit.get('addReplyFriend-'+reply.id)">{{t('common.50')}}</el-button>
                                                                    <!--取消-->
                                    ...
                                    <div class="link" v-if="answer.answerReplyList.length >state.viewNumber">
                                        <el-affix :target="'.affix-container_reply_'+answer.id" position="bottom" :offset="10" v-if="state.replyExpandOrShrink.get(answer.id)">
                                            <!-- 点击收缩 -->
                                            <span @click="telescopicReply(answer.id);" >{{t('question.640')}}</span>
                                        </el-affix>
                                        <!-- 剩余{{comment.replyList.length-state.viewNumber}}条 -->
                                        <span @click="telescopicReply(answer.id);" v-if="!state.replyExpandOrShrink.get(answer.id)">{{t('question.650',{'p1': answer.answerReplyList.length-state.viewNumber})}}</span>
                                    </div>
                                </div> 
                                        ...
                                        <el-form label-position="right" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                            ...
                                            <el-form-item :error="error.captchaValue.get('addReply-'+answer.id)" v-if="state.showCaptcha.get('addReply-'+answer.id)" class="captcha-item">
                                                <el-row align="middle">
                                                    <el-col :span="12" >
                                                        <!--placeholder="验证码"-->
                                                        <el-input  v-model="state.captchaValue['addReply-'+answer.id]" @change.native="checkCaptchaValueRules('addReply-'+answer.id)"  class="captchaInput" maxlength="4" :placeholder="t('common.70')" clearable >
                                                            <template #prefix>
                                                                <Icon name="shield-check-line" size="16px"/>
                                                            </template>
                                                        </el-input>
                                                    ...
                                                    <el-col :span="3" :offset="1">
                                                        <!--换一幅-->
                                                        <el-link type="primary" @click="replaceCaptcha('addReply-'+answer.id)" :underline="false" class="replaceCaptchaText">{{t('common.80')}}</el-link>
                                                    </el-col>
                                                </el-row>
                                            </el-form-item>
                                            <el-form-item :error="error.reply.get('addReply-'+answer.id)">
                                                <!--提交-->
                                                <el-button class="submitButton" type="primary" @mousedown.native="onAddReply(answer.id)" :disabled="state.allowSubmit.get('addReply-'+answer.id)">{{t('common.50')}}</el-button>
                                                <!--取消-->
                                                <el-button class="submitButton" type="primary" @mousedown.native="onCancelAddReply(answer.id);" plain >{{t('common.60')}}</el-button>
                                            </el-form-item>
                                        </el-form>
                                    </div>
                                </div>
                                        ...
                                        <el-form label-position="right" :ref="'formEditAnswerRef-'+answer.id" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                            <el-form-item :error="error.content.get('editAnswer-'+answer.id)">
                                                ...
                                                <div v-if="state.isEditAnswerMarkdownMap.get(answer.id)" style="width: 100%;">
                                                    <!--placeholder="请输入内容..."-->
                                                    <Editor mode="split" :editorId="'editAnswer_'+answer.id" :value="(state.editAnswerMarkdownContent[answer.id] as string)" :plugins="state.editAnswerEditorPlugin" :locale="setMarkdownLanguage()" :sanitize="answerSanitize" :placeholder="t('question.30')" @change="handleEditAnswerMarkdownChange($event,answer.id)"/>
                                                </div>
                                            </el-form-item>
                                            <el-form-item :error="error.captchaValue.get('editAnswer-'+answer.id)" v-if="state.showCaptcha.get('editAnswer-'+answer.id)" class="captcha-item">
                                                <el-row align="middle">
                                                    <el-col :span="12" >
                                                        <!--placeholder="验证码"-->
                                                        <el-input  v-model="state.captchaValue['editAnswer-'+answer.id]" @change.native="checkCaptchaValueRules('editAnswer-'+answer.id)"  class="captchaInput" maxlength="4" :placeholder="t('common.70')" clearable >
                                                            <template #prefix>
                                                                <Icon name="shield-check-line" size="16px"/>
                                                            </template>
                                                        </el-input>
                                                    ...
                                                    <el-col :span="3" :offset="1">
                                                        <!--换一幅-->
                                                        <el-link type="primary" @click="replaceCaptcha('editAnswer-'+answer.id)" :underline="false" class="replaceCaptchaText">{{t('common.80')}}</el-link>
                                                    </el-col>
                                                </el-row>
                                            </el-form-item>
                                            <el-form-item :error="error.answer.get('editAnswer-'+answer.id)" class="form-action">
                                                <!--提交-->
                                                <el-button class="submitButton" type="primary"  @mousedown.native="onEditAnswerFormSubmit(answer.id)" :disabled="state.allowSubmit.get('editAnswer-'+answer.id)">{{t('common.50')}}</el-button>
                                                <!--取消-->
                                                <el-button class="submitButton" type="primary"  @mousedown.native="onCancelEditAnswer(answer.id)" plain>{{t('common.60')}}</el-button>
                                            </el-form-item>
                                        </el-form>
                                    </div>
                                </div>
                                ...
                                <div class="tipBottom">
                                    <span class="a-toolbar">
                                        ...
                                        <a v-if="state.alreadyLikedAnswer.get(answer.id) != undefined && state.alreadyLikedAnswer.get(answer.id) == true"  @click="deleteLike(answer.id,50)">
                                            <!--赞-->
                                            <Icon name="heart-fill" size="15px" class="icon"/>{{state.answerLikeCount.get(answer.id) != undefined  && Long.fromString(String(state.answerLikeCount.get(answer.id))).gt(0) ? state.answerLikeCount.get(answer.id) : ''}}{{t('question.140')}}
                                        </a>
                                        <a v-else @click="addLike(answer.id,50)">
                                            <!--赞-->
                                            <Icon name="heart" size="15px" class="icon"/>{{state.answerLikeCount.get(answer.id) != undefined  && Long.fromString(String(state.answerLikeCount.get(answer.id))).gt(0) ? state.answerLikeCount.get(answer.id) : ''}}{{t('question.140')}}
                                        </a>
                                        ...
                                        <a v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0 && state.question.userName ==  $store.state.systemUser.userName && Long.fromString(state.question.adoptionAnswerId).eq(0)" @click="adoptionAnswer(answer.id)" >
                                            <!--采纳-->
                                            <Icon name="check-circle-solid" size="15px" class="icon"/>{{t('question.290')}}
                                        </a>
                                        
                                        <a v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0" @click="addReplyUI(answer.id)" >
                                            <!--回复-->
                                            <Icon name="reply" size="15px" class="icon"/>{{t('question.260')}}
                                        </a>
                                        ...
                                        <span class="report" v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0" @click="addReportUI(answer.id,50)">
                                            <!--举报-->
                                            <span><Icon name="error-warning-line" size="15px" class="icon"/>{{t('question.10')}}</span>
                                        </span>
                                        ...
                                        <a v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0 && answer.userName ==  $store.state.systemUser.userName && answer.adoption == false" @click="editAnswerUI(answer)">
                                            <!--编辑-->
                                            <Icon name="pencil-alt" size="15px" class="icon"/>{{t('question.270')}}
                                        </a>
                                        ...
                                        <a v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0 && answer.userName ==  $store.state.systemUser.userName" @click="onDeleteAnswer(answer.id)">
                                            <!--删除-->
                                            <Icon name="trash" size="15px" class="icon"/>{{t('question.280')}}
                                        </a>
                                    </span>
                                </div>
                            </div>
                    ...
                    <!-- 添加举报 --><!-- title="举报" -->
                    <el-dialog :title="t('question.10')" v-model="state.addReportFormView" width="612px" :draggable="true" :close-on-click-modal="false">
                            ...
                            <div class="addReport-wrap">
                                <el-form label-position="right" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                    ...
                                    <el-form-item v-if="state.show_giveReason" :error="error.reason">
                                        <!--placeholder="请填写举报理由"-->
                                        <el-input type="textarea" :autosize="{minRows: 5}" :placeholder="t('question.300')" v-model="state.reason"></el-input>
                                    </el-form-item>
                    ...
                    <div class="selectMentionUserModule">
                        <!-- title="选择用户" -->
                        <el-dialog :title="t('question.310')" :z-index="900000" v-model="state.popup_mentionUser" @close="closeMentionUser" :draggable="true" top="40vh" width="500">
                            <div class="search">
                                <el-form :inline="true" label-width="auto" @keyup.enter.native="searchMentionUser">
                                    <el-form-item :error="error.keyword">
                                        <!-- placeholder="用户账号或呢称" -->
                                        <el-input v-model.trim="state.keyword" maxlength="30" :clearable="true" :placeholder="t('question.320')"></el-input>
                                    </el-form-item>
                                    
                                    <el-form-item >
                                        <!-- 搜索 -->
                                        <el-button type="primary" size="large" class="submitButton" @click="searchMentionUser" >{{t('question.330')}}</el-button>
                                    </el-form-item>
                            ...
                            <div class="right-container" ref="right_ref">
                                ...
                                <div class="userInfo-wrap clearfix" v-if="state.question != undefined && state.question != null && Object.keys(state.question).length>0">
                                    <div class="userInfo">
                                        ...
                                        <div class="role" v-if="state.question.isStaff">
                                            <!-- 官方人员 -->
                                            <i class="staff">{{t('question.230')}}</i>	
                                        </div>
                                        <ul>
                                            <li>
                                        ...
                                        <div class="action-button" v-if="!state.question.isStaff && state.question.userName != null && state.question.userName != ''">
                                            ...
                                            <span class="action-followBox" >
                                                <el-button type="primary" @click="addFollow(state.question.userName)"><el-icon v-if="state.followText != t('question.630')" class="plusSign"><Plus /></el-icon>{{state.followText}}</el-button>
                                            </span>
                                            <span class="privateMessageBox" >
                                                 <!-- 发私信-->
                                                <el-button type="primary" plain @click="toPrivateMessage(state.question.userName)">{{t('question.370')}}</el-button>
                                            </span>
                                        </div>
                                ...
                                <div class="likeQuestionModule clearfix" v-if="state.likeQuestionList != null && state.likeQuestionList.length >0">
                                    <div class="likeQuestion">
                                        <!-- 相关问题-->
                                        <div class="title">{{t('question.380')}}</div>
                                ...
                                <div class="markdownTocModule clearfix" v-show="state.isMarkdownToc && state.markdownTocNodeInfoList != null && state.markdownTocNodeInfoList.length >0">
                                    <div class="toc">
                                        <!-- 目录-->
                                        <div class="title">{{t('question.390')}}</div>
...
<script lang="ts" setup>
    ...
    import { PageView,Question, Answer, AnswerReply, ReportType, TocNodeInfo, User, VoteTheme, VoteOption } from "@/types/index";
    import { AxiosResponse } from 'axios';
    import { letterAvatar } from '@/utils/letterAvatar';
    import Icon from "@/components/icon/Icon.vue";
    import Long from "long";
    import { escapeHtml, escapeVueHtml, unescapeHtml } from '@/utils/escape';
    import { getElementTop, getLanguageClassName, processErrorInfo } from '@/utils/tool';
    ...
    const state = reactive({
        ...
        isLoadingComplete : false,//查询‘添加答案表单‘是否加载完成
        ...
        followText: t('question.360'),//关注文本
        answerCount:'0',//回答总数
        followCount:'0',//关注总数
    ...
    const searchMentionUser = () => {
        ...
        if(state.keyword == null || state.keyword.trim() == ''){
            error.keyword = t('question.400');//请输入用户账号或呢称
            return;
        }
        proxy?.$axios({
            ...
            if(response){
                ...
                if(result != ""){
                    ...
                    if(mentionUser != null){
                        ...
                        error.keyword = t('question.410');//当前用户不存在
                    }
                }
            }
        })
                ...
                if(childNode.nodeName.toLowerCase() == "pre" ){
                    ...
                    if(firstChildNode != null && firstChildNode.getAttribute("class")!= null && firstChildNode.getAttribute("class").indexOf("language-") != -1){//Markdown代码
                        ...
                        if(firstChildNode.getAttribute("class").indexOf("language-mermaid") == -1){
                            ...
                            childNode.setAttribute("data-prismjs-copy",t('question.420'));//复制
                            childNode.setAttribute("data-prismjs-copy-error",t('question.430'));//按Ctrl+C复制
                            childNode.setAttribute("data-prismjs-copy-success",t('question.440'));//复制成功
    ...
    const addFavorite = (questionId:string) => {
        if(store.state.systemUser == null || Object.keys(store.state.systemUser).length==0){
            ElMessage({
                showClose: true,
                message: t('question.445'),//请先登录再加入收藏
                type: 'warning',
            })
            return;
        }
    ...
    const addLike = (itemId:string,module:number) => {
        if(store.state.systemUser == null || Object.keys(store.state.systemUser).length==0){
            ElMessage({
                showClose: true,
                message: t('question.470'),//请先登录再点赞
                type: 'warning',
            })
            return;
        }
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(module == 40){
                    ...
                    state.answerLikeCount.set(itemId,data)
                }else if(module == 60){
                    state.answerReplyLikeCount.set(itemId,data)
                }
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(data.allowQuestion){
                    ...
                    ElMessage({
                        showClose: true,
                        message: t('question.500'),//不允许追加提问
                        type: 'info',
                    })
                }
            }
                    ...
                    if(state.question?.allow && data.allowAnswer){
                        ...
                        if (Object.keys(state.addAnswerEditorCreateParameObject).length === 0) {//等于空
                            ...
                            if(!state.isAddAnswerMarkdown){
                                ...
                                (editorId: string)=>{
                                    //AI弹窗
                                    aiPopUp(proxy,appContext,editorId,uploadPath,'file',state.fileSystem,state.isAddAnswerMarkdown,state.addAnswerEditor,undefined)
                                });
                            }
    ...
    const adoptionAnswer = (answerId:string) => {
        ElMessageBox.confirm(t('question.520'),{//确定采纳答案?
            // type: 'warning',
            ...
            .then((response: AxiosResponse) => {
                if(response){
                    ...
                    if(JSON.parse(result.success)){
                        ElMessage({
                            showClose: true,
                            message: t('question.530'),//采纳当前答案成功
                            type: 'success',
                            onClose: ()=>{
        ...
        .then((response: AxiosResponse) => {
            ...
            if(data.allowAnswer && _answer != undefined){
                ...
                if(!data.allowAnswer){
                    ElMessage({
                        showClose: true,
                        message: t('question.540'),//不允许修改答案
                        type: 'info',
                    })
                }
            }
    ...
    const onDeleteAnswer = (answerId:string) => {
        ElMessageBox.confirm(t('question.550'),{//确定删除答案?
            // type: 'warning',
            cancelButtonText: t('common.60'),//取消
            confirmButtonText: t('common.90')//确定
        })
    ...
    const onImageExceed = (files: File[], uploadFiles: UploadUserFile[]) => {
        if(state.reportMaxImageUpload >0  && state.fileList.length >= state.reportMaxImageUpload){
            ElMessage({
                showClose: true,
                message: t('question.570'),//已达到最大图片允许上传数量
                type: 'warning',
            })
        }
    }
        ...
        .then((response: AxiosResponse) => {
            ...
            if(data.allowReport){
                ...
                if(state.reportTypeList != null && state.reportTypeList.length >0){
                    ...
                    ElMessage({
                        showClose: true,
                        message: t('question.580'),//举报分类没开启
                        type: 'info',
                    })
                }
                ...
                ElMessage({
                    showClose: true,
                    message: t('question.590'),//举报功能已关闭
                    type: 'info',
                })
            }
        })
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(data.allowReply){
                    ...
                    ElMessage({
                        showClose: true,
                        message: t('question.600'),//不允许添加回复
                        type: 'info',
                    })
                }
            }
            ...
            if(response){
                ...
                if(data.allowReply && _reply != undefined){
                    ...
                    if(!data.allowReply){
                        ElMessage({
                            showClose: true,
                            message: t('question.610'),//不允许修改回复
                            type: 'info',
                        })
                    }
                }
    ...
    const onDeleteReply = (answerId:string, replyId:string) => {
        ElMessageBox.confirm(t('question.620'),{//确定删除回复?
            // type: 'warning',
            cancelButtonText: t('common.60'),//取消
            confirmButtonText: t('common.90')//确定
        })
    ...
    const queryFollowing = (userName:string) => {
        //清空信息
        state.followText = t('question.360');//关注
        ...
        .then((response: AxiosResponse) => {
            if(response){
                const result: any = response.data;
                if(JSON.parse(result)){
                    state.followText = t('question.630');//已关注
                }
            }
        }).catch((error: any) =>{
            console.log(error);
    ...
    const addFollow = (userName:string) => {
        if(state.followText == t('question.360')){//关注
            let formData = new FormData();
    ...
    //滚动到答案窗口
    const onScrollToAnswerWindow = () => {
        (document.querySelector(".addAnswer-wrap") as any).scrollIntoView({
            behavior: "smooth"
        });
    }
    ...
    .left {
        ...
        .question-wrap{
            ...
            .questionBox{
                ...
                :deep(.content){
                    margin-top:5px;padding:20px 0px;color:$color-black-90;font-size:16px; line-height:1.75; word-wrap : break-word;word-break : break-all;min-height: 100px;
                    .cancelAccount{
                        display: inline-block;
                        vertical-align: middle;
                        padding: 6px 6px;
                ...
                :deep(.appendBox){
                    ...
                    .appendContent{
                        padding:20px 0px;color:$color-black-90;font-size:16px; line-height:1.75; word-wrap : break-word;word-break : break-all;min-height: 80px;
                        a{
                            color: #1890ff;
                        }
                        p{
