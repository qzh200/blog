...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('loginRedirect.10')+' - ' + store.state.title,//第三方重定向登录
            meta: [
                {
                    name: 'keywords',
                    content: t('loginRedirect.10')+' - ' + store.state.title//第三方重定向登录
                },{
                    name: "description",
                    content: t('loginRedirect.10')+' - ' + store.state.title//第三方重定向登录
                }
        ...
        if(response){
            ...
            if(JSON.parse(result.success)){//登录成功
                if(result.userAuthorization != null){
                    ...
                    ElMessage({
                        showClose: true,
                        message: t('loginRedirect.30'),//userAuthorization属性内容为空
                        type: 'error',
                    })
                }
            }else{
