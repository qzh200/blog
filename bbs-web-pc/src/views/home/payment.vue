                ...
                <div class="banks">
                    <div class="head">
                        <ul>
                            <li class="item1">&nbsp;</li>
                            <!-- 全部支付方式 -->
                            <li class="item2 on">{{t('payment.10')}}</li>
                            <li class="item3">&nbsp;</li>
                        </ul>
                    </div>
                    ...
                    <div class="bankSelect">
                        <el-form :model="form" ref="formRef" :rules="rules" class="form-container" size="large" label-position="top">
                            <!-- label="充值" -->
                            <el-form-item :label="t('payment.20')" :error="error.rechargeAmount" prop="rechargeAmount">
                                <!-- placeholder="请填写要充值的金额" -->
                                <el-input v-model="form.rechargeAmount" :placeholder="t('payment.30')" maxlength="12" style="width: 280px"/>
                            </el-form-item>
                        </el-form>
                    </div>
                    ...
                    <div class="payNow">
                        <!--提交-->
                        <el-button type="primary" @mousedown.native="onSubmit" :disabled="form.allowSubmit" size="large" style="width:130px">{{t('common.50')}}</el-button>
                    </div>
                </div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('payment.40')+' - ' + store.state.title,//付款
            meta: [
                {
                    name: 'keywords',
                    content: t('payment.40')+' - ' + store.state.title//付款
                },{
                    name: "description",
                    content: t('payment.40')+' - ' + store.state.title//付款
                }
    ...
    const checkRechargeAmount = (rule: any, value: any, callback: any) => {
        if (value === '') {
            return callback(new Error(t('payment.50')));//充值金额不能为空
        }else{
            if (value.trim().length > 12) {
                callback(new Error(t('payment.60')))//不能超过12位数字
            } else {   
                proxy?.$axios({
