                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            ...
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/question', query:{ questionId : scope.row.questionId,answerId:scope.row.answerId,replyId:scope.row.id,reportModule:60}}">
                                {{scope.row.content}}
                            </router-link>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" width="300">
                        <!--回复的问题-->
                        <template #header>
                            {{t('answerReplyList.20')}}
                        </template>
                        <template #default="scope">
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/question', query:{ questionId : scope.row.questionId}}">
                                {{scope.row.questionTitle}}
                            </router-link>
                        </template>
                    </el-table-column>
                    <!--label="发表时间"-->
                    <el-table-column prop="postTime" :label="t('answerReplyList.30')"  width="200" align="center"/>

                    <template #empty>
                        <!--暂无数据-->
                        <div v-if="!state.loading">{{t('answerReplyList.40')}}</div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('answerReplyList.50')+' - ' + store.state.title,//我的答案回复
            meta: [
                {
                    name: 'keywords',
                    content: t('answerReplyList.50')+' - ' + store.state.title//我的答案回复
                },{
                    name: "description",
                    content: t('answerReplyList.50')+' - ' + store.state.title//我的答案回复
                }
    ...
    const state = reactive({
        pageView: {} as PageView<AnswerReply>,//分页
        totalrecord : 0, //总记录数
		currentpage : 1, //当前页码
        totalpage : 1, //总页数
    ...
    const handlePage = (page: number) => {
		router.push({
            path: '/user/control/answerReplyList', 
            query:{page : page}
		});
        router.afterEach(() => {
    ...
    onBeforeRouteUpdate((to, from, next) => {
        if(to.name == 'answerReplyList'){
            let page:number = 1;
            if(to.query.page != undefined){
                page = parseInt(to.query.page as string);
            }
            queryAnswerReplyList(page);
        }
