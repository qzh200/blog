    ...
    <div class="main">
        <div class="main-container wrap backgroundModule main-fullScreen">
            ...
            <div class="privateMessageListModule" >
                <!-- 全部私信 -->
                <div class="privateMessageTitle"><h2>{{t('privateMessageList.10')}}</h2></div>
                    ...
                    <ol class="privateMessageList">
                        <div class="privateMessage" v-for="privateMessage in state.pageView.records">
                            ...
                            <div class="author">
                                ...
                                <cite>
                                    <!-- 此用户账号已注销 -->
                                    <span v-if="privateMessage.friendAccount == null || privateMessage.friendAccount == ''" class="cancelAccount">{{t('privateMessageList.20')}}</span>
                                    <router-link tag="a" :to="{path:'/user/control/home',query:{userName:privateMessage.friendUserName}}">
                                ...
                                <div class="tipBottom">
                                    <span class="a-toolbar">
                                        <!-- 未读 -->
                                        <span v-if="privateMessage.status == 10" class="status">{{t('privateMessageList.30')}}</span>
                                        <!-- 已读 -->
                                        <span v-if="privateMessage.status == 20" class="status alreadyRead">{{t('privateMessageList.40')}}</span>
                                        <!-- 对话 -->
                                        <router-link tag="a" :to="{path:'/user/control/privateMessageChatList',query:{friendUserName:privateMessage.friendUserName}}">
                                            <Icon name="chat-line" size="16px" class="icon"/>{{t('privateMessageList.50')}}
                                        </router-link>
                                        <!-- 删除 -->
                                        <a @click="deletePrivateMessage(privateMessage.friendUserName)"><Icon name="trash" size="16px" class="icon"/>{{t('privateMessageList.60')}}</a>
                                    </span>
                                </div>
                            </div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('privateMessageList.70')+' - ' + store.state.title,//私信列表
            meta: [
                {
                    name: 'keywords',
                    content: t('privateMessageList.70')+' - ' + store.state.title//私信列表
                },{
                    name: "description",
                    content: t('privateMessageList.70')+' - ' + store.state.title//私信列表
                }
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(JSON.parse(result.success)){//成功
                    ElMessage({
                        showClose: true,
                        message: t('privateMessageList.80'),//删除成功
                        type: 'success',
                    })
