...
<template>
    ...
    <div class="main-container wrap backgroundModule privateMessageChatTop">
        <router-link tag="a" :to="{path:'/user/control/privateMessageList'}" class="back-to-list">
             <!-- 返回私信列表 -->
            <Icon name="arrow-left-s-line" size="16px" class="icon"/>{{t('privateMessageChatList.10')}}
        </router-link>
        <div class="chatInfo">
            <b><!--与<span class="cancelAccount" v-if="state.chatUser == null">此用户账号已注销</span>
    ...
    <div class="main">
        <div class="main-container wrap">
            <div class="recentlyChatUserModule">
                <!-- 最近对话用户 -->
                <div class="title"><h3>{{t('privateMessageChatList.40')}}</h3></div>
                    ...
                    <div class="recentlyChatUser" v-for="recentlyChatUser in state.recentlyChatUserList">
                        ...
                        <div class="info">
                            ...
                            <router-link tag="a" v-if="(recentlyChatUser.friendNickname == null || recentlyChatUser.friendNickname == '') && (recentlyChatUser.friendAccount == null || recentlyChatUser.friendAccount == '')" :to="{path:'/user/control/privateMessageChatList',query:{friendUserName:recentlyChatUser.friendUserName}}" class="userName">
                                <!-- 此用户账号已注销 -->
                                <span class='cancelAccount'>{{t('privateMessageChatList.30')}}</span>
                            </router-link>
                ...
                <div class="privateMessageChatForm" v-if="form.allowSendPrivateMessage">
                    ...
                    <el-form :model="form" ref="formRef" :rules="rules" class="form-container" size="large">
                        <el-form-item :error="error.messageContent" prop="messageContent">
                            <!--placeholder="我想说的话..." -->
                            <el-input v-model="form.messageContent" :placeholder="t('privateMessageChatList.50')" show-word-limit type="textarea" :autosize="{ minRows: 6 }"/>
                        </el-form-item>
                        ...
                        <el-form-item>
                            <!--提交-->
                            <el-button type="primary" @mousedown.native="onSubmit" :disabled="form.allowSubmit" style="width:130px">{{t('common.50')}}</el-button>
                        </el-form-item>
                    </el-form>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('privateMessageChatList.60')+' - ' + store.state.title,//私信对话
            meta: [
                {
                    name: 'keywords',
                    content: t('privateMessageChatList.60')+' - ' + store.state.title//私信对话
                },{
                    name: "description",
                    content: t('privateMessageChatList.60')+' - ' + store.state.title//私信对话
                }
    ...
    const rules = reactive({
        //message: '私信内容不能为空'
        messageContent: [{ required: true, message: t('privateMessageChatList.70'), trigger: 'blur'}],
                ...
                .then((response: AxiosResponse) => {
                    if(response){
                        ...
                        if(JSON.parse(result.success)){
                             ...
                             ElMessage({
                                showClose: true,
                                message: t('privateMessageChatList.80'),//提交成功
                                type: 'success',
                            })
