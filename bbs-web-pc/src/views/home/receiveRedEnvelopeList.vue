            ...
            <div class="receiveRedEnvelopeListModule" >
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        <!-- 发红包用户 -->
                        <template #header>
                            {{t('receiveRedEnvelopeList.10')}}
                        </template>
                        <template #default="scope">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="scope.row.giveAccount == null || scope.row.giveAccount == ''" class="cancelAccount">{{t('receiveRedEnvelopeList.20')}}</div>
                    ...
                    <!-- 金额 -->
                    <el-table-column prop="amount" :label="t('receiveRedEnvelopeList.30')"  width="200" align="center"/>
                    <!-- 收取时间 -->
                    <el-table-column prop="receiveTime" :label="t('receiveRedEnvelopeList.40')"  width="200" align="center"/>
                    <template #empty>
                        <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('receiveRedEnvelopeList.50')}}</div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('receiveRedEnvelopeList.60')+' - ' + store.state.title,//收红包列表
            meta: [
                {
                    name: 'keywords',
                    content: t('receiveRedEnvelopeList.60')+' - ' + store.state.title//收红包列表
                },{
                    name: "description",
                    content: t('receiveRedEnvelopeList.60')+' - ' + store.state.title//收红包列表
                }
