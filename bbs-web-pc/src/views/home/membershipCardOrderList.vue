        ...
        <div class="main-container wrap backgroundModule main-fullScreen">
            <div class="membershipCardOrderListModule" >
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    ...
                    <!-- label="角色名称" -->
                    <el-table-column prop="roleName" :label="t('membershipCardOrderList.50')"  width="180" align="center"/>
                    <!-- label="规格名称" -->
                    <el-table-column prop="specificationName" :label="t('membershipCardOrderList.60')"  width="180" align="center"/>
                    <!-- label="数量" -->
                    <el-table-column prop="quantity" :label="t('membershipCardOrderList.70')"  width="80" align="center"/>
                    <el-table-column align="center" width="100">
                        <!-- 时长 -->
                        <template #header>
                            {{t('membershipCardOrderList.80')}}
                        </template>
                        <template #default="scope">
                            {{scope.row.duration}}
                            <span v-if="scope.row.unit == 10">{{t('membershipCardOrderList.90')}}</span><!-- 小时 -->
                            <span v-if="scope.row.unit == 20">{{t('membershipCardOrderList.100')}}</span><!-- 日 -->
                            <span v-if="scope.row.unit == 30">{{t('membershipCardOrderList.110')}}</span><!-- 月 -->
                            <span v-if="scope.row.unit == 40">{{t('membershipCardOrderList.120')}}</span><!-- 年 -->
                        </template>
                    </el-table-column>

                    <template #empty>
                        <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('membershipCardOrderList.130')}}</div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('membershipCardOrderList.140')+' - ' + store.state.title,//会员卡订单
            meta: [
                {
                    name: 'keywords',
                    content: t('membershipCardOrderList.140')+' - ' + store.state.title//会员卡订单
                },{
                    name: "description",
                    content: t('membershipCardOrderList.140')+' - ' + store.state.title//会员卡订单
                }
