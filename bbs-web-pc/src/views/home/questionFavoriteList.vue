...
<template>
    ...
    <!-- 服务导航 -->
    <ServiceNav/>
    <div class="main-container wrap backgroundModule questionFavoriteListHeadModule">
        <div class="container">
            <el-breadcrumb separator="/">
                <!-- 问题收藏用户 -->
                <el-breadcrumb-item :to="{ path: '/user/control/questionList',query:{page:router.currentRoute.value.query.questionPage != undefined ? router.currentRoute.value.query.questionPage :'1'} }">{{t('questionFavoriteList.10')}}</el-breadcrumb-item>
                <el-breadcrumb-item :to="{ path: '/question',query:{questionId:state.question?.id} }">{{state.question.title}}</el-breadcrumb-item>
            ...
            <div class="questionFavoriteListModule" >
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        <!-- 用户 -->
                        <template #header>
                            {{t('questionFavoriteList.20')}}
                        </template>
                        <template #default="scope">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="scope.row.account == null || scope.row.account == ''" class="cancelAccount">{{t('questionFavoriteList.30')}}</div>
                    ...
                    <el-table-column prop="addtime" :label="t('questionFavoriteList.40')" width="400" align="center"/>

                    <template #empty>
                         <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('questionFavoriteList.50')}}</div>
...
<script lang="ts" setup>
    ...
    import { PageView,Favorites,Question } from "@/types/index";
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('questionFavoriteList.10')+' - ' + store.state.title,//问题收藏用户
            meta: [
                {
                    name: 'keywords',
                    content: t('questionFavoriteList.10')+' - ' + store.state.title//问题收藏用户
                },{
                    name: "description",
                    content: t('questionFavoriteList.10')+' - ' + store.state.title//问题收藏用户
                }
    ...
    const handlePage = (page: number) => {
        ...
        router.push({
            path: '/user/control/questionFavoriteList', 
            query:{
                questionId: questionId,
                page : page
            }
    ...
    onBeforeRouteUpdate((to, from, next) => {
        if(to.name == 'questionFavoriteList'){
            let page:number = 1;
            ...
            queryQuestionFavoritesList(questionId,page);
...
<style scoped lang="scss">
    .questionFavoriteListHeadModule{
