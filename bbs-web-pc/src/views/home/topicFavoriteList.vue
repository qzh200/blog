    ...
    <div class="main-container wrap backgroundModule topicFavoriteListHeadModule">
        <div class="container">
            <el-breadcrumb separator="/">
                <!-- 话题收藏用户 -->
                <el-breadcrumb-item :to="{ path: '/user/control/topicList',query:{page:router.currentRoute.value.query.topicPage != undefined ? router.currentRoute.value.query.topicPage :'1'} }">{{t('topicFavoriteList.10')}}</el-breadcrumb-item>
            ...
            <div class="topicFavoriteListModule" >
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        <!-- 用户 -->
                        <template #header>
                            {{t('topicFavoriteList.20')}}
                        </template>
                        <template #default="scope">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="scope.row.account == null || scope.row.account == ''" class="cancelAccount">{{t('topicFavoriteList.30')}}</div>
                    ...
                    <!-- label="收藏时间" -->
                    <el-table-column prop="addtime" :label="t('topicFavoriteList.40')" width="400" align="center"/>

                    <template #empty>
                        <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('topicFavoriteList.50')}}</div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('topicFavoriteList.10')+' - ' + store.state.title,//话题收藏用户
            meta: [
                {
                    name: 'keywords',
                    content: t('topicFavoriteList.10')+' - ' + store.state.title//话题收藏用户
                },{
                    name: "description",
                    content: t('topicFavoriteList.10')+' - ' + store.state.title//话题收藏用户
                }
