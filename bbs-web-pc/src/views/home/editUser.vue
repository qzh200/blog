        ...
        <div class="main-container wrap backgroundModule main-fullScreen">
            <div class="editUserModule" >
                <div class="updateAvatar" >
                    <!-- 更换头像 -->
		            <div class="change" @click="updateAvatarVisible = true"><span>{{t('editUser.10')}}</span></div>
                ...
                <div class="container">
                    <el-form :rules="rules" label-position="right" ref="formRef" :model="form" class="textForm-container" size="large">
                        <el-form-item>
                            <div class="view-field">
                                <!-- 账号 -->
                                <div class="input-name">{{t('editUser.20')}}</div>
                        ...
                        <el-form-item :error="error.nickname" prop="nickname" v-if="form.user.nickname == null || form.user.nickname == ''">
                            <div class="view-field">
                                <!-- 呢称 -->
                                <div class="input-name">{{t('editUser.30')}}</div>
                                ...
                                <div class="help">
                                    ...
                                    <!-- 不设置请留空 -->
                                    <span class="help-text">{{t('editUser.40')}}</span>
                                </div>
                            </div>
                        </el-form-item>
                        <el-form-item :error="error.nickname" prop="nickname" v-if="form.user.nickname != null && form.user.nickname != ''">
                        ...
                        <el-form-item v-if="form.userRoleList != null && form.userRoleList.length >0">
                            <div class="view-field">
                                <!-- 角色 -->
                                <div class="input-name">{{t('editUser.50')}}</div>
                                    ...
                                    <table cellSpacing="0" cellPadding="0" width="100%" border="0">
                                        <tr v-for="userRole in form.userRoleList">
                                            ...
                                            <td style="text-align:left;">
                                                ...
                                                {{t('editUser.60',{'p1': userRole.validPeriodEnd})}}
                                            </td>
                                        </tr>
                                    </table>
                        ...
                        <el-form-item :error="error.oldPassword" prop="oldPassword">
                            <div class="view-field">
                                 <!-- 旧密码 -->
                                <div class="input-name">{{t('editUser.70')}}</div>
                                <div class="formInput-content">
                                    <!-- placeholder="请输入旧密码" -->
                                    <el-input v-model="form.oldPassword" :placeholder="t('editUser.80')" maxlength="20" type="password" clearable></el-input>
                                </div>
                                <div class="help">
                                    <span class="help-icon">
                                        <el-icon :size="18"><InfoFilled /></el-icon>
                                    ...
                                    <!-- 不修改请留空 -->
                                    <span class="help-text">{{t('editUser.90')}}</span>
                                </div>
                            </div>
                        </el-form-item>
                        <el-form-item :error="error.password" prop="password">
                            <div class="view-field">
                                <!-- 新密码 -->
                                <div class="input-name">{{t('editUser.100')}}</div>
                                <div class="formInput-content">
                                    <!-- placeholder="请输入新密码" -->
                                    <el-input v-model="form.password" :placeholder="t('editUser.110')" maxlength="20" type="password" clearable></el-input>
                                </div>
                                <div class="help">
                                    <span class="help-icon">
                                        <el-icon :size="18"><InfoFilled /></el-icon>
                        ...
                        <el-form-item :error="error.confirmPassword" prop="confirmPassword">
                            <div class="view-field">
                                <!-- 确认新密码 -->
                                <div class="input-name">{{t('editUser.120')}}</div>
                                <div class="formInput-content">
                                    <!-- placeholder="请输入新密码" -->
                                    <el-input v-model="form.confirmPassword" :placeholder="t('editUser.110')" maxlength="20" type="password" clearable></el-input>
                                </div>
                            </div>
                        </el-form-item>
                         <el-form-item :error="error.allowUserDynamic" >
                                <div class="view-field">
                                    <!-- 允许显示用户动态 -->
                                    <div class="input-name">{{t('editUser.130')}}</div>
                                    <div class="formInput-content">
                                        <!-- active-text="显示" inactive-text="关闭" -->
                                        <el-switch v-model="form.allowUserDynamic" :width="60" size="large" inline-prompt :active-text="t('editUser.140')" :inactive-text="t('editUser.150')"/>
                                    </div>
                                </div>
                            </el-form-item>
                        ...
                        <div v-for="(userCustom,index) in form.userCustomList">
                            ...
                            <el-form-item :error="error.userCustom.get('userCustom_'+userCustom.id)" v-if="userCustom.chooseType ==4">
                                <div class="view-field">
                                    ...
                                    <div class="formInput-content">
                                        <!-- placeholder="请选择" -->
                                        <el-select v-model="form.userBoundField[index]" :multiple="userCustom.multiple" :placeholder="t('editUser.220')" size="large">
                    ...
                    <!--提交-->
                    <el-button type="primary" size="large" style="width: 100%;" @mousedown.native="onSubmit"  :disabled="form.allowSubmit">{{t('common.50')}}</el-button>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('editUser.160')+' - ' + store.state.title,//修改个人资料
            meta: [
                {
                    name: 'keywords',
                    content: t('editUser.160')+' - ' + store.state.title//修改个人资料
                },{
                    name: "description",
                    content: t('editUser.160')+' - ' + store.state.title//修改个人资料
                }
    ...
    const checkOldPassword = (rule: any, value: any, callback: any) => {
        if(form.oldPassword != null && form.oldPassword !=''){
            if (value === '') {
                return callback(new Error(t('editUser.170')));//密码不能为空
            }else{
                if (value.trim().length < 7) {
                    callback(new Error(t('editUser.180')))//密码长度不能小于7个字符
                } else {   
                    callback();
                }
            }
    ...
    const checkPassword = (rule: any, value: any, callback: any) => {
        if(form.password != null && form.password !=''){
            if(form.oldPassword == null || form.oldPassword ==''){
                error.oldPassword = t('editUser.190');//旧密码不能为空
            }
            if (value === '') {
                return callback(new Error(t('editUser.170')));//密码不能为空
            }else{
            ...
            if (value === '') {
                ...
                if (value.trim().length < 7) {
                    ...
                    if(form.password != form.confirmPassword){
                        callback(new Error(t('editUser.200')))//两次输入密码不相等
                    }else{
                        callback();
                    }
                }
