                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            ...
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/question', query:{ questionId : scope.row.questionId,answerId:scope.row.id,reportModule:50}}">
                                {{scope.row.content}}
                            </router-link>
                        </template>
                    </el-table-column>
                    ...
                    <!--label="发表时间"-->
                    <el-table-column prop="postTime" :label="t('answerList.50')"  width="200" align="center"/>

                    <template #empty>
                        <!--暂无数据-->
                        <div v-if="!state.loading">{{t('answerList.30')}}</div>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('answerList.40')+' - ' + store.state.title,//我的答案
            meta: [
                {
                    name: 'keywords',
                    content: t('answerList.40')+' - ' + store.state.title//我的答案
                },{
                    name: "description",
                    content: t('answerList.40')+' - ' + store.state.title//我的答案
                }
    ...
    const state = reactive({
        pageView: {} as PageView<Answer>,//分页
        totalrecord : 0, //总记录数
		currentpage : 1, //当前页码
        totalpage : 1, //总页数
