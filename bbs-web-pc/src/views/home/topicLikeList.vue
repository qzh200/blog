...
<template>
    ...
    <!-- 服务导航 -->
    <ServiceNav/>
    <div class="main-container wrap backgroundModule topicLikeListHeadModule">
        <div class="container">
            <el-breadcrumb separator="/">
                <!-- 话题点赞用户 -->
                <el-breadcrumb-item :to="{ path: '/user/control/topicList',query:{page:router.currentRoute.value.query.topicPage != undefined ? router.currentRoute.value.query.topicPage :'1'} }">{{t('topicLikeList.10')}}</el-breadcrumb-item>
                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            <div class="userInfo">
                                <!-- 此用户账号已注销 -->
                                <div v-if="scope.row.account == null || scope.row.account == ''" class="cancelAccount">{{t('topicLikeList.30')}}</div>
                    ...
                    <!-- label="点赞时间" -->
                    <el-table-column prop="addtime" :label="t('topicLikeList.40')" width="400" align="center"/>

                    <template #empty>
                        <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('topicLikeList.50')}}</div>
...
<script lang="ts" setup>
    ...
    import { PageView,Like, Topic } from "@/types/index";
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('topicLikeList.10')+' - ' + store.state.title,//话题点赞用户
            meta: [
                {
                    name: 'keywords',
                    content: t('topicLikeList.10')+' - ' + store.state.title//话题点赞用户
                },{
                    name: "description",
                    content: t('topicLikeList.10')+' - ' + store.state.title//话题点赞用户
                }
    ...
    //查询话题点赞列表
    const queryTopicLikeList = (topicId:string,page: number) => {
        proxy?.$axios({
        ...
        .then((data: PageView<Like>) => {
            ...
            if(data.records != null && data.records.length >0){
                for(let i:number=0; i<data.records.length; i++){
                    let topicLike = data.records[i];
                    if(topicLike.nickname != null && topicLike.nickname !=''){
                        topicLike.avatar = letterAvatar(topicLike.nickname, 40);
                    }else{
                        topicLike.avatar = letterAvatar(topicLike.account, 40);
                    }
    ...
    const handlePage = (page: number) => {
        ...
        router.push({
            path: '/user/control/topicLikeList', 
            query:{
                topicId: topicId,
                page : page
            }
    ...
    onBeforeRouteUpdate((to, from, next) => {
        if(to.name == 'topicLikeList'){
            ...
            queryTopicLikeList(topicId,page);
            queryTopic(topicId);
        }
