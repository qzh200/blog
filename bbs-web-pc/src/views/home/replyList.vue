                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            ...
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/thread', query:{ topicId : scope.row.topicId,commentId:scope.row.commentId,replyId:scope.row.id,reportModule:30}}">
                                {{scope.row.content}}
                            </router-link>
                        </template>
                    </el-table-column>
                    <el-table-column align="center" width="300">
                        ...
                        <template #default="scope">
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/thread', query:{ topicId : scope.row.topicId}}">
                                {{scope.row.topicTitle}}
                            </router-link>
                        </template>
                    </el-table-column>
                    <!-- label="发表时间" -->
                    <el-table-column prop="postTime" :label="t('replyList.30')"  width="200" align="center"/>

                    <template #empty>
                        <!-- 暂无数据 -->
                        <div v-if="!state.loading">{{t('replyList.40')}}</div>
...
<script lang="ts" setup>
    ...
    import { PageView,Reply } from "@/types/index";
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  t('replyList.50')+' - ' + store.state.title,//我的回复
            meta: [
                {
                    name: 'keywords',
                    content: t('replyList.50')+' - ' + store.state.title//我的回复
                },{
                    name: "description",
                    content: t('replyList.50')+' - ' + store.state.title//我的回复
                }
    ...
    //查询回复列表
    const queryReplyList = (page: number) => {
        proxy?.$axios({
