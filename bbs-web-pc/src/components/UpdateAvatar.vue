...
<template>
    <div class="updateAvatar-container" >
        <div class="original-box">
            <!--原图-->
            <img ref="originalImage" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" :alt="t('updateAvatar.10')"/>
        </div>
                ...
                <div class="button-container">               
                    ...
                    <div>
                        <el-upload ref="selectImage" v-model:file-list="fileList" action="#" :auto-upload="false" :show-file-list="false" :on-change="handleChange" :accept="'image/*'">
                            <!--选择图片-->
                            <el-button type="primary" size="large" class="selectImage" plain >{{t('updateAvatar.20')}}</el-button>
                        </el-upload>
                    </div>
                    <div>
                        <!--上传-->
                        <el-button type="primary" size="large" class="uploadImage" @click="uploadAvatar" :disabled="form.allowSubmit">{{t('updateAvatar.30')}}</el-button>
                    </div>
                </div>
            </div>
        </div>
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(JSON.parse(data.success)){
                    ...
                    ElMessage({
                        showClose: true,
                        message: t('updateAvatar.50'),//上传头像成功
                        type: 'success',
                        onClose: ()=>{}
                    })
