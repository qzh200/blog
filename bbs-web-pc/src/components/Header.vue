...
<template>
    <el-affix ref="headerAffix" :offset="0" @scroll="onScroll" style="flex-shrink: 0;">
                    ...
                    <div class="shadow-container">
                        <div class="box">
                            ...
                            <div class="head-search">
                                <div class="form">
                                    <!-- 搜索 -->
                                    <input v-model.trim="state.keyword" autocomplete="off" :placeholder="t('header.30')"  @keydown.enter="submitSearch">
                            ...
                            <div class="menu">
                                <ul class="clearfix">
                                    ...
                                    <!-- 多语言切换 -->
                                    <li v-if="state.languageSwitchingOptions != null && state.languageSwitchingOptions.length >0">
                                        <el-dropdown>
                                            <div class="language-icon">
                                                <icon name="translate" size="26px"/>
                                            </div>
                                        
                                            <template #dropdown>
                                                <el-dropdown-menu>
                                                    <el-dropdown-item @click="selectLanguage(languageSwitching)" v-for="languageSwitching in state.languageSwitchingOptions"><i class="language-placeholder-icon" v-if="state.isSelectedLanguage"><icon v-if="languageSwitching.selected" name="themify-check" size="15px"/></i> {{languageSwitching.name}} </el-dropdown-item>
                                                </el-dropdown-menu>
                                            </template>
                                        </el-dropdown>
                                    </li>
                                    ...
                                    <li>
                                        <div v-if="$store.state.systemUser == null || Object.keys($store.state.systemUser).length==0">
                                            <!-- 注册 -->
                                            <li><router-link tag="a" to="/register" class="a-button">{{t('header.10')}}</router-link></li>
                                            <!-- 登录 -->
                                            <li><a  @click="loginLink" class="a-button">{{t('header.20')}}</a></li>
                                    ...
                                    <li v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0">
                                        <el-dropdown>
                                            ...
                                            <template #dropdown>
                                                <el-dropdown-menu>
                                                    <!-- 私信 -->
                                                    <el-dropdown-item @click="router.push({path: '/user/control/privateMessageList'})"><i class="icomoon-icon" ><icon name="email" size="15px"/></i> {{t('header.40')}} <el-badge :max="99" :value="state.unreadMessage.privateMessageCount" :hidden="state.unreadMessage.privateMessageCount ==0" class="custom-el-badge"/></el-dropdown-item>
                                                    <!-- 提醒 -->
                                                    <el-dropdown-item @click="router.push({path: '/user/control/remindList'})"><i class="icomoon-icon" ><icon name="bell" size="15px"/></i> {{t('header.50')}} <el-badge :max="99" :value="state.unreadMessage.remindCount" :hidden="state.unreadMessage.remindCount ==0" class="custom-el-badge"/></el-dropdown-item>
                                                    <!-- 系统通知 -->
                                                    <el-dropdown-item @click="router.push({path: '/user/control/systemNotifyList'})"><i class="icomoon-icon" ><icon name="announcement" size="15px"/></i> {{t('header.60')}} <el-badge :max="99" :value="state.unreadMessage.systemNotifyCount" :hidden="state.unreadMessage.systemNotifyCount ==0" class="custom-el-badge"/></el-dropdown-item>
                                                </el-dropdown-menu>
                                            </template>
                                    ...
                                    <div class="author" v-if="$store.state.systemUser != null && Object.keys($store.state.systemUser).length>0">
                                        <el-dropdown>
                                            ...
                                            <template #dropdown>
                                                <el-dropdown-menu>
                                                    <el-dropdown-item icon="HomeFilled"  @click="toHome()"> {{t('header.70')}} </el-dropdown-item>
                                                    <el-dropdown-item icon="Avatar" @click="updateAvatarVisible = true"> {{t('header.80')}} </el-dropdown-item>
                                                    <el-dropdown-item icon="SwitchButton" divided  @mousedown.native="onLogout"> {{t('header.90')}} </el-dropdown-item>
                                                </el-dropdown-menu>
                                            </template>
                                        </el-dropdown>
                                    </div>
        ...
        <!--登录弹出窗口-->
        <el-dialog v-model="loginVisible" width="510px" draggable :close-on-click-modal="false" modal-class="dialog-overlay" class="login-popup">
            <!-- 登录 -->
...
<script setup lang="ts">
    import { ref,getCurrentInstance, ComponentInternalInstance,reactive,onMounted,onUnmounted, watch} from 'vue'
    import { ElAffix} from 'element-plus'
    import { Column,LanguageSwitchingOptions,UnreadMessage } from "../types/index";
    ...
    const state = reactive({
        ...
        languageSwitchingOptions:[] as Array<LanguageSwitchingOptions>,//多语言切换选项
        isSelectedLanguage:false,//多语言是否有选中项
    ...
    const onScroll = (value: { scrollTop: number, fixed: boolean }):void => {
        ...
        for (let i: number = 0; i < rootElement.children.length; i++) {
            ...
            if(node.getAttribute("class")?.indexOf("el-affix--fixed") == 0){
                if((oldPosition < value.scrollTop && value.fixed && value.scrollTop >200) || (props.shrink && value.scrollTop >200)){//如果向下滚动并且在浮动状态并且滚动200像素
    ...
    const setLanguageSwitching = () => {
        ...
        state.isSelectedLanguage = false;
        if(store.state.languageSwitching != null && store.state.languageSwitching.size >0){
            ...
            for (let [key, value] of store.state.languageSwitching) {
                ...
                if(languageCode != null && languageSwitchingOptions.code == languageCode){
                    languageSwitchingOptions.selected = true;
     ...
     //监听到切换语言变化时执行刷新
     watch(() => store.state.languageSwitching, (val, old) => {
