...
axiosInstance.interceptors.request.use((config: any) => {
    ...
    if(store.state.currentLanguage == "zh"){
        config.headers['Accept-Language']= 'zh-CN';
    }else if(store.state.currentLanguage == "en"){
        config.headers['Accept-Language'] = 'en-US';
    }else{
        if(store.state.defaultLanguage == 'zh_CN'){
            config.headers['Accept-Language'] = 'zh-CN';
        }
        if(store.state.defaultLanguage == 'en_US'){
            config.headers['Accept-Language'] = 'en-US';
        }
    }
            ...
            switch (error.response.status) {
                ...
                case 401: 
                    if(!error.config.url.includes('/refreshToken')){//如果不是请求会话续期
                        ...
                        ElMessage({
                            showClose: true,
                            message: t('http.20'),//请求刷新令牌出错，请重新登录
                            type: 'error',
                        })
                        createRouter.push({
                            path : '/login'
                ...
                case 403: 
                    if(error.config.url.includes('/thread')){//话题内容页权限
                        ...
                        ElMessage({
                            showClose: true,
                            message: t('http.30'),//权限不足（您没有当前功能的操作权限）
                            type: 'error',
                        })
                    }
                ...
                case 404: 
                    ElMessage({
                        duration :0,
                        showClose: true,
                        message: t('http.40')+error.response.config.url,//找不到您尝试加载的网页（404）
                        type: 'error',
                    })
                    break;	
                case 456: //封禁IP
                ...
                ElMessage({
                    duration :0,
                    showClose: true,
                    message: t('http.60',{'p1': error.response.status}),//请求错误
                    type: 'error',
                })
            }
        }else{
