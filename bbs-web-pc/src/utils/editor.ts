...
import "../../public/common/default/pc/js/kindeditor/kindeditor-min.js";// 富文本编辑器
/**
 ...
 */
 export function createEditor(ref:any, availableTag:Array<string>, uploadPath:string, userGradeList:Array<UserGrade> | null,fileStorageSystem:number,toggleEditorCcallback:(id: string) => void,mentionCallback:(id: string) => void,aiCallback:(id: string) => void) {
	...
	//自动展开
	const autoExpand = (editor:any) => {
		timer = setInterval(() => {
			if (editor.edit && editor.cmd.doc.body) {
				//自动展开内容
				editor.autoExpandContent();

				clearInterval(timer); //销毁
				timer = null
			}
		}, 25)
	}
    ...
    let editor = KindEditor.create(ref, {
		...
		afterToggleEditor:function(){
			toggleEditorCcallback(ref.getAttribute('editorId'));
		},
		afterMention:function(){
			mentionCallback(ref.getAttribute('editorId'));
		},
		afterAi:function(){
			aiCallback(ref.getAttribute('editorId'));
		},
	});
    //自动展开内容
    //editor.autoExpandContent();
