KindEditor.plugin('hide', function(K) {
	...
	self.plugin.hideDialog = function(options) {
		...
		if(userGradeList != null && userGradeList.length >0){
			...
			userGradeHtml += '</select> '+self.lang('asSeenAbove');//以上可见
		}
		...
		html = [
				'<div style="padding:20px;height:160px;">',
					...
					'<div class="tab1" style="display:none;">',
						...
						'<label for="inputValue_10" style="min-width:34px;">'+self.lang('password')+'</label>',//密码
					...
					'<div class="tab3" style="display:none;">',
						//达到等级
						'<div class="ke-dialog-row">',
						'<label for="inputValue_30" style="min-width:60px;">'+self.lang('reachLevel')+'</label>',//达到等级
						userGradeHtml,
						'</div>',
					'</div>',
					//选项4
					'<div class="tab4" style="display:none;">',
						//积分购买
						'<div class="ke-dialog-row">',
						'<label for="inputValue_40" style="min-width:60px;">'+self.lang('payable')+'</label>',//需要支付
						'<input type="text" id="inputValue_40" class="ke-input-text ke-input-number" name="inputValue_40" value="'+options.hidePoint+'" maxlength="8" style="width:130px;" /> '+self.lang('pointsAreVisible'),//积分可见
						'</div>',
					'</div>',
					//选项5
					'<div class="tab5" style="display:none;">',
						//余额购买
						'<div class="ke-dialog-row">',
						'<label for="inputValue_50" style="min-width:60px;">'+self.lang('feesToBePaid')+'</label>',//需要支付
						'<input type="text" id="inputValue_50" class="ke-input-text ke-input-number" name="inputValue_50" value="'+options.hideAmount+'" maxlength="9" style="width:130px;" /> '+self.lang('feeToBeVisible'),//元费用可见
						'</div>',
					'</div>',
					'<div>',
						//提示
			...
			yesBtn : {
				...
				//	console.log(gradeBox.find("option:selected").text());
					...
					if(tabIndexBox == 0){//输入密码可见
						if (password == "") {
							K.popupMessage(self.lang('pleaseEnterYourPassword'));//请输入密码
							passwordBox[0].focus();
							return;
						}
						...
						if (grade == "" || !/^[0-9]*[1-9][0-9]*$/.test(grade)) {//正整数
							K.popupMessage(self.lang('pleaseSelectLevel'));//请选择等级
							if(gradeBox[0] != null){
								gradeBox[0].focus();
							}
							return;
						...
						if (point == "" || !/^[0-9]*[1-9][0-9]*$/.test(point)) {//正整数
							K.popupMessage(self.lang('pleaseEnterNumber'));//请输入大于0的数字
							pointBox[0].focus();
							return;
						}
		...
		for(var i = 0;i < items.length;i++){
			...
			if(item == "hidePassword"){
				tabTitle1 = self.lang('enterPasswordVisible');//输入密码可见
			}
			if(item == "hideComment"){
				tabTitle2 = self.lang('replyTopicVisible');//回复话题可见
			}
			if(item == "hideGrade"){
				tabTitle3 = self.lang('visibleReachLevel');//达到等级可见
			}	
			if(item == "hidePoint"){
