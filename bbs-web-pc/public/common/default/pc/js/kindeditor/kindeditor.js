...
function _getBasePath() {
	...
	//	if (/kindeditor[\w\-\.]*\.js/.test(src)) {
		if (/^(?=.*\/assets\/)(?=.*\.js).*$/.test(src) || /^(?=.*\/src\/)(?=.*\.ts).*$/.test(src)) {
			...
			return _getBasePathBefore(value)+'/common/default/pc/js/kindeditor/';
		}
	}
	return '';
}
...
function _getPageBasePath() {   
	var str = K.basePath.match(/(\S*)\/common\/default\/pc\/js\/kindeditor\//);
	if(str){
		return str[1]+"/";   
	}
	/**
...
K.options = {
	...
	items : [
		...
		'anchor', 'link', 'unlink', '|', 'about', 'toggleEditor', 'mention', 'ai'
	],
	...
	htmlTags : {
		...
		a : ['id', 'class', 'href', 'target', 'name','linktype'],
...
//	xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded"); 
	...
	if(isAuthorization){
		...
		var bbsToken = window.localStorage.getItem('bbsToken');
		if(bbsToken != null){
			var tokenObject = JSON.parse(bbsToken);
			//让每个请求携带会话token  ['Authorization']
			xmlhttp.setRequestHeader("Authorization",'Bearer '+tokenObject.accessToken+","+tokenObject.refreshToken);
		}
...
function _refreshToken(baseURL,callback){ 
	...
	baseURL,baseURL+"refreshToken", false,'',true,true);
}
...
_extend(KCmd, {
	...
	unlink : function() {
		...
		if (range.collapsed) {
			...
			if (a) {
				...
				if (a.attr('linktype') == 'mention') {//'@提及'类型不处理超链接
					return;
				}
	...
	insertmention : function(url, title) {//插入提及
		var self = this, doc = self.doc, range = self.range;
		...
		var html = '<a href="' + _escape(url) +  '" linktype="mention" target="_blank"'+'>' + _escape(title) + '</a>';

		var node =  K(html, doc);//在光标处插入节点
		self.range.insertNode(node[0]).selectNodeContents(node[0]).collapse(false);
		
		range.enlarge();//扩大边界
		
		self.range.insertNode(self.doc.createTextNode('\u200B')).collapse(false);
	...
	insertcode : function(language, code) {//插入代码
		var self = this, doc = self.doc, range = self.range;
		

		var cls = language === '' ? '' :  ' lang-' + language;
		var html = '<pre class="prettyprint' + cls + '"><code>' + K.escape(code) + '</code></pre> ';


		var codeNode =  K(html, doc);//在光标处插入节点
		self.range.insertNode(codeNode[0]).selectNodeContents(codeNode[0]).collapse(false);
		self.select();
...
function _bindNewlineEvent() {
	...
	K(doc).keydown(function(e) {
		...
		if (e.which == 8){//backspace 退格键
			self.cmd.selection(true);
			//self.cmd.select();
			var rangeTag = getRangeA(self.cmd.range);
			if (rangeTag.name == 'a' && rangeTag.attr('linktype') == 'mention') {//@提及
				
				
				//rangeTag.remove(true);
				self.cmd.unmention();
			}
		}
		//回车按下
...
KEditor.prototype = {
	...
	//插入提及
	insertMention : function(url, title) {
		this.exec('insertmention', url, title);
		return this;
	},
	//插入图片
	insertImage : function(url, title) {
		this.exec('insertimage', url, title);
		return this;
	},
	//插入代码
	insertCode : function(language, code) {
		this.exec('insertcode', language, code);
		return this;
	},
						...
						if (items[i].kind === "file" && items[i].type.indexOf('image/') !== -1) {  
							if(count ==0){//遍历第一项时创建本弹窗
								var html = ['<div style="padding:20px;">',
											...
											//图片
											self.lang('image')+' <span id="kindeditor_pasteImageUpload_imageName"></span> '+self.lang('uploading')+'..',//上传中
											'</div>',
											//提示
											'<div style="color: #909399;position: absolute;left: 21px;right:100px;bottom: 18px;">',
											''+self.lang('promptToCloseThisWindow')+'',//提示：上传完成会自动关闭本窗口
											'</div>',
											'</div>'].join('');
								var dialog = self.createDialog({
									name : name,
...
KindEditor.lang({
	...
	editCode : '修改语言',
	other : '其它',
	tipLineFeedsNotParagraphBreaks : '提示：Shift + 回车 换行不换段',
	modifyProgrammingLanguage : '修改程序语言',
	hide : '隐藏',
	image : '图片',
	uploading : '上传中',
	promptToCloseThisWindow : '提示：上传完成会自动关闭本窗口',
	embedVideo : '嵌入视频',
	uploadVideo : '上传视频',
	reachLevel : '达到等级',
	asSeenAbove : '以上可见',
	password : '密码',
	payable : '需要支付',
	pointsAreVisible : '积分可见',
	feesToBePaid : '需要支付',
	feeToBeVisible : '元费用可见',
	pleaseEnterYourPassword : '请输入密码',
	pleaseSelectLevel : '请选择等级',
	pleaseEnterNumber : '请输入大于0的数字',
	pleaseEnterAmount : '请输入大于0的金额',
	enterPasswordVisible : '输入密码可见',
	replyTopicVisible : '回复话题可见',
	visibleReachLevel : '达到等级可见',
	pointsPurchaseVisible : '积分购买可见',
	balancePurchaseVisible : '余额购买可见',
	...
	self.plugin.code = {
		add : function() {
			...
			html = ['<div style="padding:20px 20px;">',
				...
				'<option value="">'+self.lang('other')+'</option>',//其它
				'</select>',
				'</div>',
				...
				''+self.lang('tipLineFeedsNotParagraphBreaks')+'',//提示：Shift + 回车 换行不换段
				'</div>',
				'</div>'].join(''),
		...
		edit : function() {
			...
			var html = ['<div style="padding:10px 20px;">',
						...
						'<option value="" '+(selectedLang == "" ? selectedHtml :"")+'>'+self.lang('other')+'</option>',//其它
						'</select>',
						'</div>',
						'</div>'].join('');
			...
			var dialog = self.createDialog({
				name : name,
				width : 450,
				title : self.lang('modifyProgrammingLanguage'),//修改程序语言
				body : html,
				yesBtn : {
					name : self.lang('yes'),
					click : function(e) {
...
KindEditor.plugin('link', function(K) {
	...
	self.plugin.link = {
		edit : function() {
			...
			if (a && a.attr('linktype') == 'mention') {//‘@提及’类型不允许修改链接
				return;
			}
	...
	self.plugin.media = {
		edit : function() {
			var html = [
				...
				//选项1
				'<div class="tab1" style="display:none;">',    
				'<textarea name="videoCode" class="ke-textarea" style="width:449px;height:130px;" placeholder="'+self.lang('invalidVideoCodeFormat')+'"></textarea>',
				'</div>',
				//选项2          
				'<div class="tab2" style="display:none;">',
				'<div class="ke-dialog-row">',
			...
			for(var i = 0;i < items.length;i++){
				var item = items[i];
				if(item == "embedVideo"){
					tabTitle1 = self.lang('embedVideo');//嵌入视频
				}
				if(item == "uploadVideo"){
					tabTitle2 = self.lang('uploadVideo');//上传视频
				}
			}
	...
	init : function(options) {
		...
		self.div = K(options.container).html([
			...
			'<div class="ke-inline-block ke-swfupload-button ke-swfupload-desc">',
				...
				'<input type="button" class="ke-button-common ke-button" value="'+KindEditor.lang('image.addImage')+'" />',//添加图片
				'</span>',
			'</div>',
		...
		if(options.uploadModule == 10){// 0.本地 10.SeaweedFS 20.MinIO 30.阿里云OSS
			...
			uploader.on('uploadBeforeSend',function (obj,data,headers) {
				...
				if(bbsToken != null){
					...
					//让每个请求携带会话token  ['Authorization']
					$.extend(headers, {
						"Authorization": "Bearer "+tokenObject.accessToken+","+tokenObject.refreshToken
				   	});
				}
				$.extend(headers, {
...
* 提及
*******************************************************************************/
KindEditor.plugin('mention', function(K) {
	var self = this, name = 'mention';

	self.clickToolbar(name, function() {
		
		if (self.options.afterMention) {
			self.cmd.selection();
			var a = self.plugin.getSelectedLink();
			if (a && a.attr('linktype') == 'mention') {//不是‘@提及’类型链接才允许添加
				return;
			}

			self.options.afterMention.call(self);
...
* 人工智能
*******************************************************************************/
KindEditor.plugin('ai', function(K) {
	var self = this, name = 'ai';

	self.clickToolbar(name, function() {
		if (self.options.afterAi) {
			self.cmd.selection();

			self.options.afterAi.call(self);
