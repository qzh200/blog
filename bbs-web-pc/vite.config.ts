...
export default defineConfig({
    ...
    plugins: [
        ...
        prismjs({
            ...
            //  languages: 'all',
            plugins: ["line-numbers","toolbar","copy-to-clipboard"], //配置显示行号插件 copy-to-clipboard。 工具栏有新按钮时要到aiAssistantDialog.vue文件的搜索data-toolbar-order处理按钮显示
    ...
    css: {
        preprocessorOptions: {
           scss: {
                silenceDeprecations: ['legacy-js-api'],//屏蔽警告The legacy JS API is deprecated and will be removed in Dart Sass 2.0.0.
                /*
                引入var.scss全局预定义变量，
                如果引入多个文件，
                可以使用
    ...
    //assets目录新增支持的静态资源文件  (和assets.d.ts配合使用)  https://cn.vitejs.dev/config/shared-options.html#assetsinclude
    assetsInclude: ['**/*.cur'],
    build: {
        target: 'es2015',
        outDir: './dist/',
