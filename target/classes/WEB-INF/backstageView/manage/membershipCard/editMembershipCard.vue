<!-- 修改会员卡 -->
<template id="editMembershipCard-template">
	<div>
		<div class="main">
			<div class="navbar">
...
//修改会员卡
export default({
	name: 'editMembershipCard',//组件名称，keep-alive缓存需要本参数
	template : '#editMembershipCard-template',
	inject:['reload'], 
	data : function data() {
		return {
			membershipCardId:'',
	...
	created : function created() {
		...
		if(this.$route.query.membershipCardId != undefined && this.$route.query.membershipCardId != ''){
			this.membershipCardId = this.$route.query.membershipCardId;
		}
		
		this.queryEditMembershipCard();
	},
	methods : {
		 //查询修改会员卡
	    queryEditMembershipCard: function(){
	        let _self = this;
			...
			_self.$ajax.get('control/membershipCard/manage', {
			    params: {
			    	method : 'edit',
			    	membershipCardId :_self.membershipCardId,
			    }
			})
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "membershipCard"){
			    				...
			    				_self.name = membershipCard.name;
			    				if(membershipCard.subtitle != null){
			    					_self.subtitle = membershipCard.subtitle;
			    				}
			    				_self.userRoleId = membershipCard.userRoleId;
			    				_self.sort = membershipCard.sort;
			    				...
			    				if(membershipCard.introduction != null){
			    					_self.$refs.introduction.value = membershipCard.introduction;
			    				}
			    				...
			    				if(specificationList != null && specificationList.length >0){
			    					for(let i =0; i<specificationList.length; i++){
			    						...
			    						_self.specification.specificationId.push(specification.id);
			    						_self.specification.specificationName.push(specification.specificationName);
			    						_self.specification.enable.push(specification.enable);
			    						_self.specification.stock.push(specification.stock);
			    						if(specification.point != null){
			    							_self.specification.point.push(specification.point);
			    						}else{
			    							_self.specification.point.push("");
			    						}
			    						if(specification.marketPrice != null){
			    							_self.specification.marketPrice.push(specification.marketPrice);
			    						}else{
			    							_self.specification.marketPrice.push("");
			    						}
			    						if(specification.sellingPrice != null){
			    							_self.specification.sellingPrice.push(specification.sellingPrice);
			    						}else{
			    							_self.specification.sellingPrice.push("");
			    						}
			    						
			    						_self.specification.duration.push(specification.duration);
			    						_self.specification.unit.push(specification.unit);
			    						if(specification.stockStatus != null){
			    							_self.specification.stockStatus.push(specification.stockStatus);
			    						}else{
			    							_self.specification.stockStatus.push("");
			    						}
			    						if(specification.stockOccupy != null){
			    							_self.specification.stockOccupy.push(specification.stockOccupy);
			    						}else{
			    							_self.specification.stockOccupy.push("");
			    						}
			    						...
			    						_self.specification.stockStatusOptions.push(options);
			    						_self.specification.symbol.push(1);
			    						_self.specification.changeStock.push("");
			    						
			    						
			    						_self.specification.tableList.push({
								        	key: key,
								        });
	    ...
	    specification_addItem: function() {
	    	this.specification.specificationId.push("");
	    	...
	    	this.specification.stockStatus.push(0);
	    	this.specification.stockOccupy.push("");
	    	this.specification.symbol.push("");
	    	let options = [{
		    	value: 1,
		        label: '增加'
		    }, {
	    	...
	    	this.specification.stockStatusOptions.push(options);
	    	this.specification.changeStock.push("");
		...
		specification_moveUp : function(row,index) {
	        if (index >0) {
                ...
                let specificationId_upData = this.specification.specificationId[index - 1];
	        	this.specification.specificationId.splice(index - 1, 1);
                this.specification.specificationId.splice(index, 0, specificationId_upData);
                ...
                let stockStatus_upData = this.specification.stockStatus[index - 1];
	        	this.specification.stockStatus.splice(index - 1, 1);
                this.specification.stockStatus.splice(index, 0, stockStatus_upData);
                
                let stockOccupy_upData = this.specification.stockOccupy[index - 1];
	        	this.specification.stockOccupy.splice(index - 1, 1);
                this.specification.stockOccupy.splice(index, 0, stockOccupy_upData);
                
                let changeStock_upData = this.specification.changeStock[index - 1];
	        	this.specification.changeStock.splice(index - 1, 1);
                this.specification.changeStock.splice(index, 0, changeStock_upData);
                
                let symbol_upData = this.specification.symbol[index - 1];
	        	this.specification.symbol.splice(index - 1, 1);
                this.specification.symbol.splice(index, 0, symbol_upData);
                
                let stockStatusOptions_upData = this.specification.stockStatusOptions[index - 1];
	        	this.specification.stockStatusOptions.splice(index - 1, 1);
                this.specification.stockStatusOptions.splice(index, 0, stockStatusOptions_upData);
		...
		specification_moveDown : function(row,index) {
	        if ((index + 1) < this.specification.tableList.length) {
                ...
                let specificationId_downData = this.specification.specificationId[index + 1];
                this.specification.specificationId.splice(index + 1, 1);
                this.specification.specificationId.splice(index, 0, specificationId_downData);
                ...
                let stockStatus_downData = this.specification.stockStatus[index + 1];
                this.specification.stockStatus.splice(index + 1, 1);
                this.specification.stockStatus.splice(index, 0, stockStatus_downData);
                
                let stockOccupy_downData = this.specification.stockOccupy[index + 1];
                this.specification.stockOccupy.splice(index + 1, 1);
                this.specification.stockOccupy.splice(index, 0, stockOccupy_downData);
                
                let changeStock_downData = this.specification.changeStock[index + 1];
                this.specification.changeStock.splice(index + 1, 1);
                this.specification.changeStock.splice(index, 0, changeStock_downData);
                
                let symbol_downData = this.specification.symbol[index + 1];
                this.specification.symbol.splice(index + 1, 1);
                this.specification.symbol.splice(index, 0, symbol_downData);
                
                let stockStatusOptions_downData = this.specification.stockStatusOptions[index + 1];
                this.specification.stockStatusOptions.splice(index + 1, 1);
                this.specification.stockStatusOptions.splice(index, 0, stockStatusOptions_downData);
		...
		specification_removeItem : function(row,index) {
			...
			this.specification.specificationId.splice(index, 1);
			...
			this.specification.stockStatus.splice(index, 1);
			this.specification.stockOccupy.splice(index, 1);
			this.specification.changeStock.splice(index, 1);
			this.specification.symbol.splice(index, 1);
			this.specification.stockStatusOptions.splice(index, 1);
		...
		submitForm : function() {
			...
			formData.append('membershipCardId', _self.membershipCardId);
			...
			for(let i=0; i<_self.specification.tableList.length; i++){
				...
				let specificationId = _self.specification.specificationId[i];
				...
				let stockStatus = _self.specification.stockStatus[i];
				let symbol = _self.specification.symbol[i];
				let changeStock = _self.specification.changeStock[i];
				...
				if(stockStatus == 0){
					...
					formData.append('stockStatus', symbol);
				}
