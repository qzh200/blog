    ...
    <div class="main">
        <div class="search">
            <el-form :inline="true" @keyup.enter.native="submitForm">
                ...
                <el-form-item :error="error.userTypeCode"  v-if="state.searchType ==1">
                    
                    <el-select v-model="state.userTypeCode" size="large" class="custom-select-width" @change="selectedUserType" >
                        <el-option v-for="(value, key)  in state.userTypeGroup" :label="value" :value="key"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item :error="error.keyword" style="width: 280px;" v-if="state.searchType ==1">
                    <el-tooltip :content="'请输入'+state.userTypeGroup[state.userTypeCode]" placement="top">
                        <el-input v-model.trim="state.keyword" maxlength="100" style="width: 100%;"  :placeholder="state.userTypeGroup[state.userTypeCode]"></el-input>
                    </el-tooltip>
                </el-form-item>
...
<script lang="ts" setup>
    ...
    const state = reactive({
        ...
        userTypeGroup: {'10':'账号','20':'呢称','30':'用户名','40':'手机号','50':'邮箱'} as any,
    ...
    //选中用户类型
    const selectedUserType = (val:any) => {
    }
    ...
    const queryUserList = () => {
        ...
        proxy?.$axios({
            ...
            params: {
                ...
                userTypeCode:state.userTypeCode,
    ...
    onMounted(() => {
        ...
        if(router.currentRoute.value.query.userTypeCode != undefined && router.currentRoute.value.query.userTypeCode != ''){
			state.userTypeCode = router.currentRoute.value.query.userTypeCode as string;
		}
