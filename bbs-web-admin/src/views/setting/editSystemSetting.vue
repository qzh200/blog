    ...
    <div class="main" >
        <div class="data-form label-width-blank systemSettingModule" >
            <el-form label-width="270px"  @submit.native.prevent>
                ...
                <el-form-item label="允许注册账号类型" :error="error.allowRegisterAccountType" v-show="state.activeTag == 10">
                    <el-select ref="allowRegisterAccountTypeRef"  v-model="state.allowRegisterAccountTypeCodeGroup" size="large" @remove-tag="processRemoveAllowRegisterAccountType" @click="loadAllowRegisterAccountType"  style="width: 100%;" multiple :placeholder="state.select_allowRegisterAccountType_placeholder" >
                        <el-option v-for="item in state.allowRegisterAccountTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                    <div class="form-help" >空则不允许注册账号</div>
                </el-form-item>
                <el-form-item label="允许登录账号类型" :error="error.allowLoginAccountType" v-show="state.activeTag == 10">
                    <el-select ref="allowLoginAccountTypeRef"  v-model="state.allowLoginAccountTypeCodeGroup" size="large" @remove-tag="processRemoveAllowLoginAccountType" @click="loadAllowLoginAccountType"  style="width: 100%;" multiple :placeholder="state.select_allowLoginAccountType_placeholder" >
                        <el-option v-for="item in state.allowLoginAccountTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
            ...
            <div class="selectSupportAccountTypeModule">
                <!-- 允许注册账号类型 -->
                <el-dialog title="允许注册账号类型" v-model="state.popup_allowRegisterAccountType" :draggable="true"  @close="closeAllowRegisterAccountType">
                    <div class="supportAccountTypeNavigation">
                        <div class="tab-content">
                            <div class="tab-pane">
                                <span :class="state.selectedAllowRegisterAccountTypeClass.get(key)" @click="selectedAllowRegisterAccountType(key,value)"  v-for="([key, value], index) in state.supportAccountType.entries()" >
                                    {{value}}
                                </span>
                            </div>
                        </div>
                ...
                <!-- 允许登录账号类型 -->
                <el-dialog title="允许登录账号类型" v-model="state.popup_allowLoginAccountType" :draggable="true"  @close="closeAllowLoginAccountType">
                    <div class="supportAccountTypeNavigation">
                        <div class="tab-content">
                            <div class="tab-pane">
                                <span :class="state.selectedAllowLoginAccountTypeClass.get(key)" @click="selectedAllowLoginAccountType(key,value)"  v-for="([key, value], index) in state.supportAccountType.entries()" >
                                    {{value}}
                                </span>
                            </div>
                        </div>
...
<script lang="ts" setup>
    ...
    const allowRegisterAccountTypeRef = ref()
    const allowLoginAccountTypeRef = ref()

    //账户类型Select 选择器选项
    interface AccountTypeOptionsItem {
        value: number;
        label: string;
    }
    ...
    const state = reactive({
        ...
        supportAccountType:new Map<number,string>(),//支持账号类型
        popup_allowRegisterAccountType:false,//是否显示‘允许注册账号类型‘弹窗
        selectedAllowRegisterAccountTypeClass:new Map<number,string>(),//选中‘允许注册账号类型‘样式
        select_allowRegisterAccountType_placeholder: '请选择',
        allowRegisterAccountTypeCodeGroup :[] as any,//‘允许注册账号类型‘代码组
        allowRegisterAccountTypeOptions: [] as Array<AccountTypeOptionsItem>,//Select 选择器‘允许注册账号类型’数据

        popup_allowLoginAccountType:false,//是否显示‘允许登录账号类型‘弹窗
        selectedAllowLoginAccountTypeClass:new Map<number,string>(),//选中‘允许登录账号类型‘样式
        select_allowLoginAccountType_placeholder: '请选择',
        allowLoginAccountTypeCodeGroup :[] as any,//‘允许登录账号类型‘代码组
        allowLoginAccountTypeOptions: [] as Array<AccountTypeOptionsItem>,//Select 选择器‘允许登录账号类型’数据
    ...
    const closeAllowRegisterAccountType = () => {
        state.popup_allowRegisterAccountType = false;

    }
    //设置注册账号类型 -- 选中‘账号类型‘样式
    const setAllowRegisterAccountTypeClass = () => {
        for(let [code,name] of state.supportAccountType.entries()){
            state.selectedAllowRegisterAccountTypeClass.set(code,"supportAccountType-tag");
            if(state.allowRegisterAccountTypeCodeGroup != null && state.allowRegisterAccountTypeCodeGroup.length >0){
                for(let i = 0; i <state.allowRegisterAccountTypeCodeGroup.length; i++){
                    let code = state.allowRegisterAccountTypeCodeGroup[i];
                    state.selectedAllowRegisterAccountTypeClass.set(code,"supportAccountType-tag selected");
    ...
    //允许注册账号类型 -- 选中‘账号类型‘
    const selectedAllowRegisterAccountType = (code:number,name:string) => {
       
        //判断是否重复选择,如果重复则取消选择
        if(state.allowRegisterAccountTypeOptions != null && state.allowRegisterAccountTypeOptions.length >0){
            for(var i=0; i<state.allowRegisterAccountTypeOptions.length; i++){
                var selectedAllowRegisterAccountTypeOptions = state.allowRegisterAccountTypeOptions[i];
                if(selectedAllowRegisterAccountTypeOptions.value == code){
                    //删除‘账号类型‘
                    deleteAllowRegisterAccountType(selectedAllowRegisterAccountTypeOptions.value);
                    setAllowRegisterAccountTypeClass();
                    return;
                }
            }
        ...
        state.allowRegisterAccountTypeOptions.push(o);
        
        state.allowRegisterAccountTypeCodeGroup.push(code);
        
        state.select_allowRegisterAccountType_placeholder = "";
        setAllowRegisterAccountTypeClass();
    }

    //允许注册账号类型 -- 删除‘账号类型‘
    const deleteAllowRegisterAccountType = (code:number) => {
		if(state.allowRegisterAccountTypeOptions != null && state.allowRegisterAccountTypeOptions.length >0){
            for(let i=0; i<state.allowRegisterAccountTypeOptions.length; i++){
                let selectedTag = state.allowRegisterAccountTypeOptions[i];
                if(selectedTag.value == code){
                    state.allowRegisterAccountTypeOptions.splice(i, 1);
                    
                    for(let j=0; j<state.allowRegisterAccountTypeCodeGroup.length; j++){
                        if(state.allowRegisterAccountTypeCodeGroup[j] == code){
                            state.allowRegisterAccountTypeCodeGroup.splice(j, 1);
                            break;
                        }
                        
                    }
                    
                    if(state.allowRegisterAccountTypeCodeGroup.length ==0){
                        state.select_allowRegisterAccountType_placeholder = "请选择";
                    }
                    return;
                }
            }
    ...
    //允许注册账号类型 -- 处理删除‘账号类型‘
    const processRemoveAllowRegisterAccountType = (val:any) => {
        if(state.allowRegisterAccountTypeCodeGroup.length ==0){
    ...
    //允许注册账号类型 -- 加载‘账号类型‘
    const loadAllowRegisterAccountType = () => {
        state.popup_allowRegisterAccountType = true;
        allowRegisterAccountTypeRef.value.blur();//使Select选择器失去焦点，并隐藏下拉框
        if(state.allowRegisterAccountTypeCodeGroup.length ==0){
            state.allowRegisterAccountTypeOptions.length = 0;//清空	
        }
        setAllowRegisterAccountTypeClass();
    ...
    const closeAllowLoginAccountType = () => {
        state.popup_allowLoginAccountType = false;

    }
    //设置登录账号类型 -- 选中‘账号类型‘样式
    const setAllowLoginAccountTypeClass = () => {
        for(let [code,name] of state.supportAccountType.entries()){
            state.selectedAllowLoginAccountTypeClass.set(code,"supportAccountType-tag");
            if(state.allowLoginAccountTypeCodeGroup != null && state.allowLoginAccountTypeCodeGroup.length >0){
                for(let i = 0; i <state.allowLoginAccountTypeCodeGroup.length; i++){
                    let code = state.allowLoginAccountTypeCodeGroup[i];
                    state.selectedAllowLoginAccountTypeClass.set(code,"supportAccountType-tag selected");
    ...
    //允许登录账号类型 -- 选中‘账号类型‘
    const selectedAllowLoginAccountType = (code:number,name:string) => {
       
        //判断是否重复选择,如果重复则取消选择
        if(state.allowLoginAccountTypeOptions != null && state.allowLoginAccountTypeOptions.length >0){
            for(var i=0; i<state.allowLoginAccountTypeOptions.length; i++){
                var selectedAllowLoginAccountTypeOptions = state.allowLoginAccountTypeOptions[i];
                if(selectedAllowLoginAccountTypeOptions.value == code){
                    //删除‘账号类型‘
                    deleteAllowLoginAccountType(selectedAllowLoginAccountTypeOptions.value);
                    setAllowLoginAccountTypeClass();
                    return;
                }
            }
        ...
        state.allowLoginAccountTypeOptions.push(o);
        
        state.allowLoginAccountTypeCodeGroup.push(code);
        
        state.select_allowLoginAccountType_placeholder = "";
        setAllowLoginAccountTypeClass();
    }

    //允许登录账号类型 -- 删除‘账号类型‘
    const deleteAllowLoginAccountType = (code:number) => {
		if(state.allowLoginAccountTypeOptions != null && state.allowLoginAccountTypeOptions.length >0){
            for(let i=0; i<state.allowLoginAccountTypeOptions.length; i++){
                let selectedTag = state.allowLoginAccountTypeOptions[i];
                if(selectedTag.value == code){
                    state.allowLoginAccountTypeOptions.splice(i, 1);
                    
                    for(let j=0; j<state.allowLoginAccountTypeCodeGroup.length; j++){
                        if(state.allowLoginAccountTypeCodeGroup[j] == code){
                            state.allowLoginAccountTypeCodeGroup.splice(j, 1);
                            break;
                        }
                        
                    }
                    
                    if(state.allowLoginAccountTypeCodeGroup.length ==0){
                        state.select_allowLoginAccountType_placeholder = "请选择";
                    }
                    return;
                }
            }
    ...
    //允许登录账号类型 -- 处理删除‘账号类型‘
    const processRemoveAllowLoginAccountType = (code:number) => {
        if(state.allowLoginAccountTypeCodeGroup.length ==0){
    ...
    //允许登录账号类型 -- 加载‘账号类型‘
    const loadAllowLoginAccountType = () => {
        state.popup_allowLoginAccountType = true;
        allowLoginAccountTypeRef.value.blur();//使Select选择器失去焦点，并隐藏下拉框
        if(state.allowLoginAccountTypeCodeGroup.length ==0){
            state.allowLoginAccountTypeOptions.length = 0;//清空	
        }
        setAllowLoginAccountTypeClass();
    }
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			...
			    			if(key == "systemSetting"){
                                ...
                                state.supportAccountType.clear();
                                for (let code in mapData[key]) {
                                    state.supportAccountType.set(parseInt(code),mapData[key][code])
                                }
                            }
			    			...
			    			if(systemSetting != null){
			    				...
			    				state.allowRegisterAccountTypeCodeGroup.length =0;
                                state.allowRegisterAccountTypeOptions.length =0;
			    					...
			    					A:for(let i = 0; i <systemSetting.allowRegisterAccountTypeCodeList.length; i++){
                                        ...
                                        for(let [code,name] of state.supportAccountType.entries()){
                                            if(code == allowRegisterAccountTypeCode){
                                                state.allowRegisterAccountTypeCodeGroup.push(allowRegisterAccountTypeCode);
                                                let obj:any =new Object();
                                                obj.value = allowRegisterAccountTypeCode;
                                                obj.label = name;
                                                state.allowRegisterAccountTypeOptions.push(obj);
                                                continue A;
                                            }
                                        }
                                ...
                                state.allowLoginAccountTypeCodeGroup.length =0;
                                state.allowLoginAccountTypeOptions.length =0;
			    					...
			    					A:for(let i = 0; i <systemSetting.allowLoginAccountTypeCodeList.length; i++){
                                        ...
                                        for(let [code,name] of state.supportAccountType.entries()){
                                            if(code == allowLoginAccountTypeCode){
                                                state.allowLoginAccountTypeCodeGroup.push(allowLoginAccountTypeCode);
                                                let obj:any =new Object();
                                                obj.value = allowLoginAccountTypeCode;
                                                obj.label = name;
                                                state.allowLoginAccountTypeOptions.push(obj);
                                                continue A;
                                            }
                                        }
                                ...
                                if(systemSetting.aiAssistant_submitQuantity != null){
			    					state.aiAssistant_submitQuantity = systemSetting.aiAssistant_submitQuantity;
			    				}
    ...
    const submitForm = () => {
        ...
        if(state.aiAssistant_submitQuantity != null){
            formData.append('aiAssistant_submitQuantity', state.aiAssistant_submitQuantity);
        }
...
/** 选择账号类型 **/
.selectSupportAccountTypeModule {
    background: #fff;
    .supportAccountTypeNavigation{
        font-size: 14px;
        .tab-content{
            margin: 8px 0px 8px 0px;
            cursor:pointer;
        }
        .supportAccountType-tag {
