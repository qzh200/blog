 ...
 * @param anchor 锚点
 */
export function anchorJump(anchor:string) {
    if(document.querySelector("[aria-label*='源码编辑']")){//版块代码 --> 源码编辑
        //获取滚动窗口
        let position:any = (document.querySelector("[aria-label*='源码编辑']") as any).querySelector(".el-dialog")

		
		let height = (document.querySelector(anchor) as any).offsetTop;
        
		//装有源码编辑功能的容器高度
		let forumSource_container_height = (document.querySelector(".forumSource-container") as any).offsetHeight;
