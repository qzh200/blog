<!-- 修改版块 -->
<template id="editForum-template">
	<div>
		<div class="main" >
			...
			<div class="data-form label-width-blank forumModule" >
				<el-form label-width="auto"  @submit.native.prevent>
					<el-form-item label="版块类型" >
						{{forum.forumType}} -- {{forum.forumChildType}}
					</el-form-item>
							...
							<el-table-column label="表单" >
								<template #default="scope">	
									...
									<el-form-item label="图片" :error="error.collection_Forum_AdvertisingRelated_imageList.collection_image_imagePath.get(scope.$index)" >
										...
										<el-upload action="#" :ref="el => collection_Forum_AdvertisingRelated_imageList.uploadImg_elementNodes[scope.$index]=el" :file-list="collection_Forum_AdvertisingRelated_imageList.photoList.get(scope.row.key)"  list-type="picture-card" :auto-upload="false" :on-change="advertising_imageList_handleChange" :accept="'image/*'">
										    <template #default>
						...
						<el-dialog title="选择帮助" v-model="collection_Forum_HelpRelated_RecommendHelp.help_form">
							<div class="dialog-nav-search">
								<el-form :inline="true" label-width="auto" @submit.native.prevent   >
									...
									<el-form-item >
									    <el-button type="primary" @click="collection_Forum_HelpRelated_RecommendHelp_helpPage(1)" >搜索</el-button>
									</el-form-item>
...
//修改版块
export default({
	name: 'editForum',//组件名称，keep-alive缓存需要本参数
	template : '#editForum-template',
	inject:['reload'], 
	data : function data() {
		return {
			layoutId :'',
	...
	　　　　handler(newValue, oldValue) {
				this.$nextTick(function(){//渲染结束再监听
					if(this.forumChildType == '在线帮助列表' && this.displayType == '单层'){
	...
	created : function created() {
		...
		if(this.$route.query.forumId != undefined && this.$route.query.forumId != ''){
			this.forumId = this.$route.query.forumId;
		}
		
		
		
		this.queryEditForumUI();
	},
	methods : {
		...
		advertising_imageList_handleImageRemove(file,tableIndex) {
			...
			this.collection_Forum_AdvertisingRelated_imageList.collection_image_path[tableIndex] = '';
		...
		collection_Forum_AdvertisingRelated_imageList_removeItem : function(row,index) {
			...
			this.collection_Forum_AdvertisingRelated_imageList.photoList.delete(row.key);
			...
			.then(function (response) {
			    ...
			    if(result){
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		//如果没有选择，则默认选中第一项
			    		if(_self.monolayer_Forum_HelpRelated_Help.monolayer_help_more == ''){
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "layout"){
			    				...
			    				_self.forum = mapData[key];
			    				
			    				_self.forumType = _self.forum.forumType;
			    				_self.forumChildType = _self.forum.forumChildType;
			    				_self.name = _self.forum.name;//版块标题
			    				_self.module = _self.forum.module;//版块模板
	       		 				_self.invokeMethod = _self.forum.invokeMethod;//调用方式
			    				
			    				let stringArray = _self.forum.module.split("_");
			    				...
			    				_self.forumCodeNode_remark = mapData[key];
			    				...
			    				if(collection_Forum_AdvertisingRelated_imageList != null && collection_Forum_AdvertisingRelated_imageList.length >0){
			    					for(let i =0; i<collection_Forum_AdvertisingRelated_imageList.length; i++){
			    						...
			    						_self.collection_Forum_AdvertisingRelated_imageList.collection_image_name.push(imageList.image_name);
										_self.collection_Forum_AdvertisingRelated_imageList.collection_image_link.push(imageList.image_link);
										_self.collection_Forum_AdvertisingRelated_imageList.collection_image_path.push(imageList.image_filePath + _self.dirName + "/" + imageList.image_fileName);
			    						
			    						
			    						if(imageList.image_fileName != ""){
			    							_self.collection_Forum_AdvertisingRelated_imageList.photoList.set(key,[{
								                url: imageList.image_filePath + _self.dirName + "/" + imageList.image_fileName,
								            }]);
			    						...
			    						_self.collection_Forum_AdvertisingRelated_imageList.tableList.push({
								        	key: key,//随机数
								        });
			    				...
			    				if(page_Forum_TopicRelated_Topic != null){
							        ...
							        if(page_Forum_TopicRelated_Topic.topic_tagId != null){
							        	_self.page_Forum_TopicRelated_Topic.tagIdGroup.push(page_Forum_TopicRelated_Topic.topic_tagId);
										_self.page_Forum_TopicRelated_Topic.page_topic_tagId = page_Forum_TopicRelated_Topic.topic_tagId;
							        	let obj =new Object();
										obj.value = page_Forum_TopicRelated_Topic.topic_tagId;
										obj.label = page_Forum_TopicRelated_Topic.topic_tagName;
										_self.page_Forum_TopicRelated_Topic.tagOptions.length = 0;//清空
										_self.page_Forum_TopicRelated_Topic.tagOptions.push(obj);
										
							        }
									_self.page_Forum_TopicRelated_Topic.page_topic_tag_transferPrameter = page_Forum_TopicRelated_Topic.topic_tag_transferPrameter;
									_self.page_Forum_TopicRelated_Topic.page_topic_sort = page_Forum_TopicRelated_Topic.topic_sort;
									_self.page_Forum_TopicRelated_Topic.page_topic_maxResult = page_Forum_TopicRelated_Topic.topic_maxResult;
									_self.page_Forum_TopicRelated_Topic.page_topic_pageCount = page_Forum_TopicRelated_Topic.topic_pageCount;
			    				...
			    				if(page_Forum_CommentRelated_Comment != null){
			    					...
			    					_self.page_Forum_CommentRelated_Comment.page_comment_sort = page_Forum_CommentRelated_Comment.comment_sort;
									_self.page_Forum_CommentRelated_Comment.page_comment_maxResult = page_Forum_CommentRelated_Comment.comment_maxResult;
									_self.page_Forum_CommentRelated_Comment.page_comment_pageCount = page_Forum_CommentRelated_Comment.comment_pageCount;
			    			...
			    			}else if(key == "page_Forum_TopicRelated_TopicEssence"){
			    				let page_Forum_TopicRelated_TopicEssence = mapData[key];
			    				if(page_Forum_TopicRelated_TopicEssence != null){
							      	_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_sort = page_Forum_TopicRelated_TopicEssence.topicEssence_sort;
									_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_maxResult = page_Forum_TopicRelated_TopicEssence.topicEssence_maxResult;
									_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_pageCount = page_Forum_TopicRelated_TopicEssence.topicEssence_pageCount;
			    				...
			    				if(page_Forum_QuestionRelated_Question != null){
							        ...
							        if(page_Forum_QuestionRelated_Question.question_tagId != null){
							        	_self.page_Forum_QuestionRelated_Question.tagIdGroup.push(page_Forum_QuestionRelated_Question.question_tagId);
										_self.page_Forum_QuestionRelated_Question.page_question_tagId = page_Forum_QuestionRelated_Question.question_tagId;
							        	let obj =new Object();
										...
										_self.page_Forum_QuestionRelated_Question.tagOptions.length = 0;//清空
										_self.page_Forum_QuestionRelated_Question.tagOptions.push(obj);
										
							        }
									_self.page_Forum_QuestionRelated_Question.page_question_tag_transferPrameter = page_Forum_QuestionRelated_Question.question_tag_transferPrameter;
									if(page_Forum_QuestionRelated_Question.question_filterCondition != null){
										_self.page_Forum_QuestionRelated_Question.page_question_filterCondition = page_Forum_QuestionRelated_Question.question_filterCondition;						
									
									}
									_self.page_Forum_QuestionRelated_Question.page_question_filterCondition_transferPrameter = page_Forum_QuestionRelated_Question.question_filterCondition_transferPrameter;
									_self.page_Forum_QuestionRelated_Question.page_question_sort = page_Forum_QuestionRelated_Question.question_sort;
									_self.page_Forum_QuestionRelated_Question.page_question_maxResult = page_Forum_QuestionRelated_Question.question_maxResult;
									_self.page_Forum_QuestionRelated_Question.page_question_pageCount = page_Forum_QuestionRelated_Question.question_pageCount;
			    				...
			    				if(page_Forum_AnswerRelated_Answer != null){
			    					...
			    					_self.page_Forum_AnswerRelated_Answer.page_answer_sort = page_Forum_AnswerRelated_Answer.answer_sort;
									_self.page_Forum_AnswerRelated_Answer.page_answer_maxResult = page_Forum_AnswerRelated_Answer.answer_maxResult;
									_self.page_Forum_AnswerRelated_Answer.page_answer_pageCount = page_Forum_AnswerRelated_Answer.answer_pageCount;
			    				...
			    				if(collection_Forum_QuestionRelated_LikeQuestion != null){
									_self.collection_Forum_QuestionRelated_LikeQuestion.collection_likeQuestion_maxResult = collection_Forum_QuestionRelated_LikeQuestion.likeQuestion_maxResult;
			    				...
			    				if(page_Forum_RedEnvelopeRelated_ReceiveRedEnvelopeUser != null){
									_self.page_Forum_RedEnvelopeRelated_ReceiveRedEnvelopeUser.page_receiveRedEnvelopeUser_maxResult = page_Forum_RedEnvelopeRelated_ReceiveRedEnvelopeUser.receiveRedEnvelopeUser_maxResult;
									_self.page_Forum_RedEnvelopeRelated_ReceiveRedEnvelopeUser.page_receiveRedEnvelopeUser_sort = page_Forum_RedEnvelopeRelated_ReceiveRedEnvelopeUser.receiveRedEnvelopeUser_sort;
			    				...
			    				if(monolayer_Forum_HelpRelated_Help != null){
							        ...
							        if(monolayer_Forum_HelpRelated_Help.help_helpTypeId != null){
							        	_self.monolayer_Forum_HelpRelated_Help.helpTypeIdGroup.push(monolayer_Forum_HelpRelated_Help.help_helpTypeId);
					
							        	_self.monolayer_Forum_HelpRelated_Help.monolayer_help_helpTypeId = monolayer_Forum_HelpRelated_Help.help_helpTypeId;
							        	let helpType_obj =new Object();
										...
										_self.monolayer_Forum_HelpRelated_Help.helpTypeOptions.length = 0;//清空
										_self.monolayer_Forum_HelpRelated_Help.helpTypeOptions.push(helpType_obj);
							        }
			    					...
			    					if(monolayer_Forum_HelpRelated_Help.help_more != null){
						    	    	...
						    	    	_self.monolayer_Forum_HelpRelated_Help.moreOptions.push(obj);
						    	    	_self.monolayer_Forum_HelpRelated_Help.monolayer_help_more = monolayer_Forum_HelpRelated_Help.help_more;
			    					}
			    					
			    					
			    					
			    					_self.monolayer_Forum_HelpRelated_Help.monolayer_help_helpType_transferPrameter = monolayer_Forum_HelpRelated_Help.help_helpType_transferPrameter;
									
									_self.monolayer_Forum_HelpRelated_Help.monolayer_help_quantity = monolayer_Forum_HelpRelated_Help.help_quantity;
									
									_self.monolayer_Forum_HelpRelated_Help.monolayer_help_maxResult = monolayer_Forum_HelpRelated_Help.help_maxResult;
									_self.monolayer_Forum_HelpRelated_Help.monolayer_help_pageCount = monolayer_Forum_HelpRelated_Help.help_pageCount;
									_self.monolayer_Forum_HelpRelated_Help.monolayer_help_sort = monolayer_Forum_HelpRelated_Help.help_sort;
			    				...
			    				if(page_Forum_HelpRelated_Help != null){
							        ...
							        if(page_Forum_HelpRelated_Help.help_helpTypeId != null){
							        	_self.page_Forum_HelpRelated_Help.helpTypeIdGroup.push(page_Forum_HelpRelated_Help.help_helpTypeId);
					
							        	_self.page_Forum_HelpRelated_Help.page_help_helpTypeId = page_Forum_HelpRelated_Help.help_helpTypeId;
							        	let helpType_obj =new Object();
										...
										_self.page_Forum_HelpRelated_Help.helpTypeOptions.length = 0;//清空
										_self.page_Forum_HelpRelated_Help.helpTypeOptions.push(helpType_obj);
							        }
							        
			    					
			    					_self.page_Forum_HelpRelated_Help.page_help_helpType_transferPrameter = page_Forum_HelpRelated_Help.help_helpType_transferPrameter;
									
									_self.page_Forum_HelpRelated_Help.page_help_maxResult = page_Forum_HelpRelated_Help.help_maxResult;
									_self.page_Forum_HelpRelated_Help.page_help_pageCount = page_Forum_HelpRelated_Help.help_pageCount;
									_self.page_Forum_HelpRelated_Help.page_help_sort = page_Forum_HelpRelated_Help.help_sort;
			    				...
			    				if(collection_Forum_HelpRelated_RecommendHelp != null){
									...
									for(let i = 0; i<recommendHelpList.length; i++){
										...
										_self.collection_Forum_HelpRelated_RecommendHelp.tableList.push({
											id: recommendHelp.id,
			    				...
			    				if(entityBean_Forum_CustomForumRelated_CustomHTML != null){
									...
									if(html_content != null && html_content != ''){
										_self.$refs.entityBean_customForum_htmlContent.value = html_content;
									}
			    				}
			    				...
			    				if(entityBean_Forum_CustomForumRelated_CustomHTML != null){
									...
									if(searchWordList != null && searchWordList.length>0){
										for(let i =0; i<searchWordList.length; i++){
											_self.collection_Forum_SystemRelated_SearchWord.formOptions.push({
		...
		submitForm : function() {
			...
			if(_self.forumId != null){
				formData.append('forumId', _self.forumId);
			}
			...
			if(_self.forumType == '话题' && _self.forumChildType == '话题精华列表' && _self.displayType == '分页'){
				if(_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_sort != null){
					formData.append('page_topicEssence_sort', _self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_sort);
				}
				if(_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_maxResult != null){
					formData.append('page_topicEssence_maxResult', _self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_maxResult);
				}
				if(_self.page_Forum_TopicRelated_TopicEssence.page_topicEssence_pageCount != null){
			...
			if(_self.forumType == '广告' && _self.forumChildType == '图片广告' && _self.displayType == '集合'){
				for(let i=0; i<_self.collection_Forum_AdvertisingRelated_imageList.tableList.length; i++){
					...
					let image_path = _self.collection_Forum_AdvertisingRelated_imageList.collection_image_path[i];
					...
					if(fileList != null && fileList.length >0){
						for(let i=0; i<fileList.length; i++){
							...
							if(image_path == file.url){//如果没有上传文件
								break;
							}
