...
public class ZipUtil {
    ...
    private static void pack(ZipArchiveOutputStream out, String sourceDir, String excludeDirectory, String entryPath) {
        ...
        try {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (excludeDirectoryPath != null
                        && StringUtils.startsWithIgnoreCase(files[i].getAbsolutePath(), excludeDirectoryPath)) {
                    // 跳过排除目录下的文件
                    continue;
                }
                ...
                if (files[i].isDirectory()) {
                    if (files[i].listFiles().length > 0) {
                        pack(out, files[i].getAbsolutePath(), excludeDirectory,
     ...
     * @param entryPath 压缩内文件逻辑路径。如static/
     * 
     *                  public static void pack(File source, String targetZip,String
     *                  entryPath){
     *                  if(!entryPath.endsWith(File.separator)&&entryPath != null &&
     *                  !entryPath.isEmpty()){
     *                  entryPath+=File.separator;
     *                  }
     *                  ZipArchiveOutputStream out = null;
     *                  try {
     *                  out = new ZipArchiveOutputStream(new
