	...
	public String execute(ModelMap model,String jump,
		...
		if(jump != null && !"".equals(jump.trim())){
			...
			if(_tagId .equals(-1L)){//如果为-1,则是后台管理页面下载，不检查权限
				if(isAjax == true){
					redirect = fileManage.createSignLink(link,URLEncoder.encode(fileName,"UTF-8"),systemSetting.getFileSecureLinkSecret(),systemSetting.getFileSecureLinkExpire());
				}else{
		...
		if(isAjax == true){
    		...
    		if(redirect == null || "".equals(redirect.trim())){
    			error.put("message", "下载失败");
    			returnValue.put("success", "false");
    			returnValue.put("error", error);
    			
    		}else{
    			returnValue.put("success", "true");
    			returnValue.put("redirect", redirect);
    		}
