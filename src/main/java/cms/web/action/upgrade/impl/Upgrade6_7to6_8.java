 ...
 *
 */
public class Upgrade6_7to6_8 {
	private static final Logger logger = LogManager.getLogger(Upgrade6_7to6_8.class);
    ...
    public static void run(String upgradeId){
    	...
    	for(int i =0; i< 100; i++){
    		...
    		if(upgradeSystem.getRunningStatus()>=40 && upgradeSystem.getRunningStatus()<200){
    			...
    			updateSQL_emailSetting(upgradeService);
    			upgradeService.addLog(upgradeId, JsonUtils.toJSONString(new UpgradeLog(new Date(),"修改emailsetting表字段supportedMailboxes为longtext类型",1))+",");
    ...
    /**
   	 * 修改emailsetting表字段supportedMailboxes为longtext类型
   	 * @param upgradeService
   	 */
    private static void updateSQL_emailSetting(UpgradeService upgradeService){
    	String updateSQL = "ALTER TABLE emailsetting MODIFY supportedMailboxes LONGTEXT";
