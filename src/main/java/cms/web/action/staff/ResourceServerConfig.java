...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        ...
        // .csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse());//将CSRF令牌存储在自定义Cookie中
        // CookieServerCsrfTokenRepository 不允许JS读取可以设置new CookieCsrfTokenRepository()
    ...
    public void configure(ResourceServerSecurityConfigurer resources) throws Exception {
        ...
        resources
                ...
                .tokenExtractor(myBearerTokenExtractor());// token获取方式,默认BearerTokenExtractor
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        if (allowedOrigins != null && !"".equals(allowedOrigins.trim())) {
            ...
            configuration.setAllowedHeaders(Arrays.asList("Origin", "X-Requested-With", "Content-Type", "Cache-Control",
                    "Accept", "Authorization", "BBS-XSRF-TOKEN", "Set-Cookie"));
            // 暴露哪些头部信息（因为跨域访问默认不能获取全部头部信息）
            // configuration.addExposedHeader("*");
            configuration.addExposedHeader("Message");// Message为cms.web.action.staff.WebSecurityConfig.java的accessDeniedHandler方法返回的消息头。
